...
class Hooks(_Verbose):
    ...
    def add_module(self, name):
        ...
        d[name] = self.new_module(name)
        m = self.new_module(name)
        return m
    ...
    def load_module(self, name, stuff):
        ...
        try:
            ...
            if type == C_EXTENSION:
                ...
                raise ImportError, 'Unrecognized module type (%s) for %s' % (`type`, name)
        finally:
            if file:
                file.close()
    ...
    def load_module(self, name, stuff):
        ...
        if type == PKG_DIRECTORY:
            ...
            if inittype not in (PY_COMPILED, PY_SOURCE):
                ...
                raise ImportError, 'Bad type (%s) for __init__ module in package %s' % (`inittype`, name)
...
class BasicModuleImporter(_Verbose):
    ...
    def __init__(self, loader = None, verbose = VERBOSE):
        ...
        if not loader:
            pass
        self.loader = ModuleLoader(None, verbose)
...
def install(importer = None):
    global current_importer
    if not importer and default_importer:
        pass
    current_importer = ModuleImporter()
