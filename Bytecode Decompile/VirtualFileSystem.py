...
class VirtualFileSystem(FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_mount_ptrVirtualFileSystem_ptrConstFilename_atomicstring_int(self, physicalFilename, mountPoint, flags):
        returnValue = libpandaexpress._inPJoxt0Y1P(self.this, physicalFilename.this, mountPoint, flags)
        return returnValue

    def __overloaded_mount_ptrVirtualFileSystem_ptrMultifile_atomicstring_int(self, multifile, mountPoint, flags):
        returnValue = libpandaexpress._inPJoxtcQsl(self.this, multifile.this, mountPoint, flags)
        return returnValue

    def __overloaded_unmount_ptrVirtualFileSystem_ptrConstFilename(self, physicalFilename):
        returnValue = libpandaexpress._inPJoxt_kpP(self.this, physicalFilename.this)
        return returnValue

    def __overloaded_unmount_ptrVirtualFileSystem_ptrMultifile(self, multifile):
    ...
    def __overloaded_resolveFilename_ptrConstVirtualFileSystem_ptrFilename_ptrConstDSearchPath_atomicstring(self, filename, searchpath, defaultExtension):
        returnValue = libpandaexpress._inPJoxt_tIn(self.this, filename.this, searchpath.this, defaultExtension)
        return returnValue

    def __overloaded_resolveFilename_ptrConstVirtualFileSystem_ptrFilename_ptrConstDSearchPath(self, filename, searchpath):
            ...
            if isinstance(_args[0], Multifile.Multifile):
                if isinstance(_args[1], types.StringType):
                    if isinstance(_args[2], types.IntType):
                        return self.__overloaded_mount_ptrVirtualFileSystem_ptrMultifile_atomicstring_int(_args[0], _args[1], _args[2])
                    else:
                ...
                if isinstance(_args[0], Filename.Filename):
                    if isinstance(_args[1], types.StringType):
                        if isinstance(_args[2], types.IntType):
                            return self.__overloaded_mount_ptrVirtualFileSystem_ptrConstFilename_atomicstring_int(_args[0], _args[1], _args[2])
                        else:
                            raise TypeError, 'Invalid argument 2, expected one of: <types.IntType> '
                    else:
                        raise TypeError, 'Invalid argument 1, expected one of: <types.StringType> '
    ...
    def unmount(self, *_args):
        ...
        if numArgs == 1:
            import Multifile, Filename
            if isinstance(_args[0], Multifile.Multifile):
                return self.__overloaded_unmount_ptrVirtualFileSystem_ptrMultifile(_args[0])
            else:
                if isinstance(_args[0], Filename.Filename):
                    return self.__overloaded_unmount_ptrVirtualFileSystem_ptrConstFilename(_args[0])
                else:
                    raise TypeError, 'Invalid argument 0, expected one of: <Multifile.Multifile> <Filename.Filename> '
        else:
        ...
        if numArgs == 2:
            ...
            if isinstance(_args[0], Filename.Filename):
                ...
                if isinstance(_args[1], DSearchPath.DSearchPath):
                    return self.__overloaded_resolveFilename_ptrConstVirtualFileSystem_ptrFilename_ptrConstDSearchPath(_args[0], _args[1])
                else:
                ...
                if isinstance(_args[0], Filename.Filename):
                    ...
                    if isinstance(_args[1], DSearchPath.DSearchPath):
                        if isinstance(_args[2], types.StringType):
                            return self.__overloaded_resolveFilename_ptrConstVirtualFileSystem_ptrFilename_ptrConstDSearchPath_atomicstring(_args[0], _args[1], _args[2])
                        else:
