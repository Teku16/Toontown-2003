...
class VirtualFile(TypedReferenceCount.TypedReferenceCount, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_ls_ptrConstVirtualFile_ptrOstream(self, out):
        returnValue = libpandaexpress._inPJoxtkWDV(self.this, out.this)
        return returnValue

    def __overloaded_ls_ptrConstVirtualFile(self):
        returnValue = libpandaexpress._inPJoxt52an(self.this)
        return returnValue

    def __overloaded_lsAll_ptrConstVirtualFile_ptrOstream(self, out):
        returnValue = libpandaexpress._inPJoxtqJmn(self.this, out.this)
        return returnValue

    def __overloaded_lsAll_ptrConstVirtualFile(self):
    ...
    def lsAll(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self.__overloaded_lsAll_ptrConstVirtualFile()
        else:
            if numArgs == 1:
                import Ostream
                if isinstance(_args[0], Ostream.Ostream):
                    return self.__overloaded_lsAll_ptrConstVirtualFile_ptrOstream(_args[0])
                else:
    ...
    def ls(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self.__overloaded_ls_ptrConstVirtualFile()
        else:
            if numArgs == 1:
                import Ostream
                if isinstance(_args[0], Ostream.Ostream):
                    return self.__overloaded_ls_ptrConstVirtualFile_ptrOstream(_args[0])
                else:
                    raise TypeError, 'Invalid argument 0, expected one of: <Ostream.Ostream> '
            else:
                raise TypeError, 'Invalid number of arguments: ' + `numArgs` + ', expected one of: 0 1 '