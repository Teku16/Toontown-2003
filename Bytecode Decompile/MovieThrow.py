...
import DirectNotifyGlobal, whrandom, MovieCamera, MovieUtil
notify = DirectNotifyGlobal.directNotify.newCategory('MovieThrow')
hitSoundFiles = (
 'AA_tart_only.mp3', 'AA_slice_only.mp3', 'AA_slice_only.mp3', 'AA_slice_only.mp3', 'AA_slice_only.mp3', 'AA_wholepie_only.mp3')
tPieLeavesHand = 2.7
tPieHitsSuit = 3.0
tSuitDodges = 2.45
ratioMissToHit = 1.5
...
pieNames = (
 'tart', 'fruitpie-slice', 'creampie-slice', 'fruitpie', 'creampie', 'birthday-cake')
...
def __getSoundTrack(level, hitSuit, node=None):
    ...
    throwTrack = Track([(2.6, SoundInterval(throwSound, node=node))])
    if hitSuit:
        hitSound = globalBattleSoundCache.getSound(hitSoundFiles[level])
        hitTrack = Track([(tPieLeavesHand, SoundInterval(hitSound, node=node))])
        return MultiTrack([throwTrack, hitTrack])
    else:
        return throwTrack
