...
class Splash(NodePath):
    ...
    def createTrack(self, rate=1):
        ...
        self.track = Sequence(Func(self.show), Parallel(self.ripples.track, Sequence(Func(self.splashdown.show), Func(self.splashdown.play, 'splashdown'), Wait(animDuration), Func(self.splashdown.hide)), Sequence(Func(self.pSystem.show), Func(self.particles.induceLabor), Func(self.pSystem.start, self), Wait(2.2), Func(self.pSystem.hide), Func(self.pSystem.disable))), Func(self.hide), name='splashdown-%d-track' % self.trackId)
