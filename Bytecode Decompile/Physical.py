...
class Physical(TypedReferenceCount.TypedReferenceCount, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_constructor_ptrConstPhysical(self, copy):
        self.this = libpandaphysics._inP9fJJqW30(copy.this)
        self.userManagesMemory = 1

    def __overloaded_constructor_int_bool(self, ttlObjects, preAlloc):
        self.this = libpandaphysics._inP9fJJvRxK(ttlObjects, preAlloc)
        self.userManagesMemory = 1

    def __overloaded_constructor_int(self, ttlObjects):
            ...
            if numArgs == 1:
                if isinstance(_args[0], types.IntType):
                    ...
                    if isinstance(_args[0], Physical):
                        return self.__overloaded_constructor_ptrConstPhysical(_args[0])
                    else:
                ...
                if numArgs == 2:
                    if isinstance(_args[0], types.IntType):
                        if isinstance(_args[1], types.IntType):
                            return self.__overloaded_constructor_int_bool(_args[0], _args[1])
                        else:
