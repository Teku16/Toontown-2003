...
import PandaObject, DirectNotifyGlobal, StateData, PublicWalk, DownloadForceAcknowledge, ZoneUtil, FriendsListManager, ToontownGlobals, HouseGlobals, Localizer, Task
...
class Place(PandaObject.PandaObject, StateData.StateData, FriendsListManager.FriendsListManager):
    ...
    def periodTimerExpired(self):
        ...
        if toonbase.localToon.book.isEntered:
            ...
            toonbase.localToon.b_setAnimState('CloseBook', 1, callback=self.__handlePeriodTimerBookClose)
        else:
            toonbase.localToon.b_setAnimState('TeleportOut', 1, self.__handlePeriodTimerExitTeleport)
    ...
    def handleWalkDone(self, doneStatus):
        ...
        if mode == 'StickerBook':
            ...
            if mode == 'Options':
                self.last = self.fsm.getCurrentState().getName()
                self.fsm.request('stickerBook', [toonbase.localToon.optionsPage])
            else:
    ...
    def enterStickerBookGUI(self):
        ...
        toonbase.localToon.startSleepWatch(self.__handleFallingAsleep)
    ...
    def __handleFallingAsleep(self, task):
        toonbase.localToon.book.exit()
        toonbase.localToon.b_setAnimState('CloseBook', 1, callback=self.__handleFallingAsleepBookClose)
        return Task.done
    ...
    def __handleBook(self):
        ...
        if bookStatus['mode'] == 'close':
            ...
            if bookStatus['mode'] == 'teleport':
                ...
                toonbase.localToon.collisionsOff()
                toonbase.localToon.b_setAnimState('CloseBook', 1, callback=self.handleBookCloseTeleport, extraArgs=[zoneId, zoneId])
            else:
                if bookStatus['mode'] == 'exit':
                    self.exitTo = bookStatus.get('exitTo')
                    toonbase.localToon.b_setAnimState('CloseBook', 1, callback=self.__handleBookCloseExit)
                else:
                    if bookStatus['mode'] == 'gohome':
                        zoneId = bookStatus['hood']
                        toonbase.localToon.collisionsOff()
                        toonbase.localToon.b_setAnimState('CloseBook', 1, callback=self.__handleBookCloseGoHome, extraArgs=[zoneId])

    def handleBookCloseTeleport(self, hoodId, zoneId):
        if hasattr(self, 'fsm'):
            self.fsm.request('DFA', [{'loader': ZoneUtil.getBranchLoaderName(zoneId), 'where': ZoneUtil.getToonWhereName(zoneId), 'how': 'teleportIn', 'hoodId': hoodId, 'zoneId': zoneId, 'shardId': None, 'avId': -1}])
        return

    def __handleBookCloseExit(self):
        toonbase.localToon.b_setAnimState('TeleportOut', 1, self.__handleBookExitTeleport, [0])
    ...
    def __handleBookCloseGoHome(self, curZoneId):
        hoodId = ToontownGlobals.MyEstate
        if hasattr(self, 'fsm'):
            self.fsm.request('DFA', [{'loader': 'estateLoader', 'where': 'estate', 'how': 'teleportIn', 'hoodId': hoodId, 'zoneId': -1, 'shardId': None, 'avId': -1}])
        return
    ...
    def enterTeleportOut(self, requestStatus, callback):
        toonbase.localToon.laffMeter.start()
        toonbase.localToon.b_setAnimState('TeleportOut', 1, callback, [requestStatus])
    ...
    def goHomeFailed(self, task):
        ...
        self.doneStatus['zone'] = toonbase.localToon.lastHood
        self.fsm.request('deathAck', [self.doneStatus])
        return Task.done
    ...
    def requestTeleport(self, hoodId, zoneId, shardId, avId):
        ...
        if hoodId == ToontownGlobals.MyEstate:
            ...
            whereId = 'estate'
        if hasattr(self, 'fsm'):
            self.fsm.request('DFA', [{'loader': loaderId, 'where': whereId, 'how': 'teleportIn', 'hoodId': hoodId, 'zoneId': zoneId, 'shardId': shardId, 'avId': avId}])
