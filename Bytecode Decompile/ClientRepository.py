...
import Task, DirectNotifyGlobal, ClientDistClass, CRCache, DirectObject
    ...
    def httpConnectCallback(self, ch, serverList, serverIndex, hasProxy, successCallback, successArgs, failureCallback, failureArgs):
        if ch.isConnectionReady():
            ...
            if serverIndex < len(serverList):
                ...
                ch.spawnTask(name='connect-to-server', callback=self.httpConnectCallback, extraArgs=[ch, serverList, serverIndex + 1, hasProxy, successCallback, successArgs, failureCallback, failureArgs])
            else:
                if failureCallback:
