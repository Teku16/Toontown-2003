...
import OnscreenText, StateData, ToontownDialog, FSM, State, DirectNotifyGlobal, Task, Localizer, TTAccount, GuiScreen, InputCheck, copy, PythonUtil
...
class BillingScreen(StateData.StateData, GuiScreen.GuiScreen):
    ...
    ActiveEntryColor = (
     1, 0.9, 0.1, 1)
    InactiveEntryColor = (0.8, 0.8, 0.8, 1)
    ...
    showLegalBlurb = 0
    countryCodes = [
     'US', 'CA', 'AU', 'UK', 'AF', 'AL', 'DZ', 'AS', 'AD', 'AO', 'AI', 'AQ', 'AG', 'AR', 'AM', 'AW', 'AT', 'AZ', 'BS', 'BH', 'BD', 'BB', 'BY', 'BE', 'BZ', 'BJ', 'BM', 'BT', 'BO', 'BA', 'BW', 'BV', 'BR', 'IO', 'BN', 'BG', 'BF', 'BI', 'KH', 'CM', 'CV', 'KY', 'CF', 'TD', 'CL', 'CN', 'CX', 'CC', 'CO', 'KM', 'CG', 'CK', 'CR', 'CI', 'HR', 'CU', 'CY', 'CZ', 'CS', 'DK', 'DJ', 'DM', 'DO', 'TP', 'EC', 'EG', 'SV', 'GQ', 'ER', 'EE', 'ET', 'FK', 'FO', 'FJ', 'FI', 'FR', 'FX', 'GF', 'PF', 'TF', 'GA', 'GM', 'GE', 'DE', 'GH', 'GI', 'GB', 'GR', 'GL', 'GD', 'GP', 'GU', 'GT', 'GN', 'GW', 'GY', 'HT', 'HM', 'HN', 'HK', 'HU', 'IS', 'IN', 'ID', 'IR', 'IQ', 'IE', 'IL', 'IT', 'JM', 'JP', 'JO', 'KZ', 'KE', 'KI', 'KP', 'KR', 'KW', 'KG', 'LA', 'LV', 'LB', 'LS', 'LR', 'LY', 'LI', 'LT', 'LU', 'MO', 'MK', 'MG', 'MW', 'MY', 'MV', 'ML', 'MT', 'MH', 'MQ', 'MR', 'MU', 'YT', 'MX', 'FM', 'MD', 'MC', 'MN', 'MS', 'MA', 'MZ', 'MM', 'NA', 'NR', 'NP', 'NL', 'AN', 'NT', 'NC', 'NZ', 'NI', 'NE', 'NG', 'NU', 'NF', 'MP', 'NO', 'OM', 'PK', 'PW', 'PA', 'PG', 'PY', 'PE', 'PH', 'PN', 'PL', 'PT', 'PR', 'QA', 'RE', 'RO', 'RU', 'RW', 'GS', 'KN', 'LC', 'VC', 'WS', 'SM', 'ST', 'SA', 'SN', 'SC', 'SL', 'SG', 'SK', 'SI', 'Sb', 'SO', 'ZA', 'ES', 'LK', 'SH', 'PM', 'SD', 'SR', 'SJ', 'SZ', 'SE', 'CH', 'SY', 'TW', 'TJ', 'TZ', 'TH', 'TG', 'TK', 'TO', 'TT', 'TN', 'TR', 'TM', 'TC', 'TV', 'UG', 'UA', 'AE', 'UY', 'UM', 'SU', 'UZ', 'VU', 'VA', 'VE', 'VN', 'VG', 'VI', 'WF', 'EH', 'YE', 'YU', 'ZR', 'ZM', 'ZW']
    state50Codes = [
     'AL', 'AK', 'AR', 'AZ', 'CA', 'CO', 'CT', 'DE', 'FL', 'GA', 'HI', 'IA', 'ID', 'IL', 'IN', 'KS', 'KY', 'LA', 'MA', 'MD', 'ME', 'MI', 'MN', 'MO', 'MS', 'MT', 'NE', 'NC', 'ND', 'NH', 'NJ', 'NM', 'NV', 'NY', 'OH', 'OK', 'OR', 'PA', 'RI', 'SC', 'SD', 'TN', 'TX', 'UT', 'VA', 'VT', 'WA', 'WI', 'WV', 'WY', 'DC']
    territoryStateCodes = [
     'AS', 'GU', 'MP', 'PR', 'VI', 'FPO', 'APO', 'MH', 'PW', 'FM']
    ...
    def load(self):
        ...
        lineHeight = 0.15 * masterScale * 0.9
        if self.showLegalBlurb:
            lineHeight *= 0.85 / 0.95
        buttonScale = 1.15 * masterScale
        buttonLineHeight = 0.14 * masterScale
        entryWidth = 15 * 0.75
        entryFrameSize = (-0.2, entryWidth + 0.2, -0.6, 1.3)
        labelX = -0.17
        entryX = -0.12
        scrolledListZOffset = 0.005
        ...
        self.visaLogo.setPos(0.49, 0, -0.295)
        self.visaLogo.setScale(0.51)
        if self.preferVisa:
            self.visaLogo.show()
        else:
            self.visaLogo.hide()
        self.cogIcon = DirectLabel(parent=self.frame, relief=None, pos=(-1.12, 0, 0.8), scale=0.2, image=cogIcons.find('**/MoneyIcon'))
        linePos = 0.85
        self.titleLabel = DirectLabel(parent=self.frame, relief=None, pos=(0, 0, linePos), text=Localizer.BillingScreenTitle, text_scale=textScale, text_font=getSuitFont(), text_fg=(0.1, 0.2, 0.1, 1))
        linePos -= lineHeight
        ...
        linePos -= lineHeight
        scaleDown = 0.75
        oldValues = [textScale, entryScale, lineHeight, buttonScale, buttonLineHeight, entryWidth, entryFrameSize]
        textScale *= scaleDown * 1.1
        entryScale *= scaleDown
        lineHeight *= scaleDown * 1.1
        ...
        self.emailEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        self.emailConfirmEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        self.address1Entry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        self.address2Entry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        self.cityEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        iS = 14.0
        self.defaultCountryScale = 0.8
        self.countryControl = DirectScrolledList(parent=self.frame, relief=None, items=countries, image=loginGui.find('**/cogCountryPanel'), image_scale=(iS * 1.3, iS, iS), image_pos=(0.65, 0, 0.15), scale=optionMenuScale, incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(18.2478, 1, -15.09), incButton_pos=(5.26773, 0, -0.0801019), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(18.2478, 1, 15.09), decButton_pos=(5.26773, 0, 0.603461), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(0.231214, 0.0, linePos + scrolledListZOffset), itemFrame_pos=(0, 0, 0), itemFrame_scale=self.defaultCountryScale, itemFrame_relief=None, itemFrame_textMayChange=1)
        ...
        hS = 1.5
        bX = 5.05272
        self.stateControl = DirectScrolledList(parent=self.frame, relief=None, items=states, image=loginGui.find('**/cogCountryPanel'), image_scale=(iS * hS, iS, iS), image_pos=imagePos, incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(bS * hS, bS, -bS), incButton_pos=(bX, 0, -0.0682188), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(bS * hS, bS, bS), decButton_pos=(bX, 0, 0.52128), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(0.208342, 0.0, linePos + scrolledListZOffset), scale=0.068, itemFrame_pos=(-0.110268, 0, 0), itemFrame_scale=itemFrameScale, itemFrame_relief=None)
        ...
        bS = 12.55
        iS = 12.0
        self.defaultCAProvinceScale = 0.8
        CA_provinces = map(lambda pCode: Localizer.BillingScreenCanadianProvinces[pCode], self.canadianProvinceCodes)
        self.caProvinceControl = DirectScrolledList(parent=self.frame, relief=None, items=CA_provinces, image=loginGui.find('**/cogCountryPanel'), image_scale=(iS * 1.3, iS, iS), image_pos=(0.5, 0, 0.13), incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(bS * 1.27, bS, -bS), incButton_pos=(4.45884, 0, -0.0682188), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(bS * 1.27, bS, bS), decButton_pos=(4.45884, 0, 0.52128), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(0.156309, 0.0, linePos + scrolledListZOffset), scale=0.068, itemFrame_pos=(0, 0, 0), itemFrame_scale=itemFrameScale, itemFrame_relief=None)
        ...
        self.provinceEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        ...
        self.zipCodeEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1)
        self.zipCodeEntry.label = self.zipCodeLabel
        linePos -= lineHeight
        if self.showLegalBlurb:
            ccTypeControlHeightRatio = 0.1
        else:
        ...
        bS = 14
        bX = 3.81904
        self.ccTypeControl = DirectScrolledList(parent=self.frame, relief=None, items=[Localizer.BillingScreenCCTypeInitialText] + Localizer.BillingScreenCreditCardTypes, image=loginGui.find('**/cogCountryPanel'), image_scale=(iS, iS, iS), image_pos=(0.5, 0, 0.15), scale=optionMenuScale, incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(bS, bS, -bS), incButton_pos=(bX, 0, -0.0371349), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(bS, bS, bS), decButton_pos=(bX, 0, 0.603461), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(0.125294, 0.0, linePos + scrolledListZOffset), itemFrame_pos=(0, 0, 0), itemFrame_scale=self.defaultCountryScale, itemFrame_relief=None, itemFrame_textMayChange=1)
        ...
        self.ccNumberEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, width=entryWidth, scale=entryScale, pos=(entryX, 0.0, linePos), numLines=1, focus=0, cursorKeys=1)
        ...
        imagePos = (0.5, 0, 0.13)
        imageScale = 12
        self.ccMonthControl = DirectScrolledList(parent=self.frame, relief=None, items=self.months, image=loginGui.find('**/cogMonthPanel'), image_scale=imageScale, image_pos=imagePos, incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(bS, bS, bS), incButton_pos=(1.38257, 0, 0.52128), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(bS, bS, -bS), decButton_pos=(1.38257, 0, -0.0682188), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(-0.062, 0.0, linePos + scrolledListZOffset), scale=0.068, itemFrame_pos=(0, 0, 0), itemFrame_scale=itemFrameScale, itemFrame_relief=None)
        ...
        years = map(str, years)
        self.ccYearControl = DirectScrolledList(parent=self.frame, relief=None, items=years, image=loginGui.find('**/cogYearPanel'), image_scale=imageScale, image_pos=imagePos, incButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), incButton_relief=None, incButton_scale=(bS, bS, bS), incButton_pos=(2.12466, 0, 0.52128), incButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), decButton_image=(loginGui.find('**/cogPanelArrowUp'), loginGui.find('**/cogPanelArrowDwn'), loginGui.find('**/cogPanelArrowRllvr'), loginGui.find('**/cogPanelArrowUp')), decButton_relief=None, decButton_scale=(bS, bS, -bS), decButton_pos=(2.12466, 0, -0.0682188), decButton_image3_color=Vec4(0.5, 0.5, 0.5, 1), pos=(0.213567, 0.0, linePos + scrolledListZOffset), scale=0.068, itemFrame_pos=(0, 0, 0), itemFrame_scale=itemFrameScale, itemFrame_relief=None)
        linePos -= lineHeight
        ...
        self.nameOnCardEntry = DirectEntry(parent=self.frame, relief=SUNKEN, borderWidth=(0.1, 0.1), frameColor=self.EntryFrameColor, frameSize=entryFrameSize, scale=entryScale, pos=(entryX, 0.0, linePos), width=entryWidth, numLines=1, focus=0, cursorKeys=1, command=self.__handleNameOnCard)
        self.nameOnCardEntry.label = self.nameOnCardLabel
        linePos -= lineHeight
        priceTextScale = 0.09
        priceShadowOffset = (0.04, 0.04)
        self.pricingLabel = DirectLabel(parent=self.frame, relief=None, pos=(-0.11, 0, -0.657), text=Localizer.BillingScreenPricing, text_scale=priceTextScale, text_align=TextNode.ACenter, text_fg=(0.2, 0.2, 0.2, 1), text_wordwrap=42)
        priceColor = (
         0, 0.9, 0, 1)
        self.priceFirstMonth = DirectLabel(parent=self.pricingLabel, relief=None, pos=(-0.85, 0, 0), text='$%s' % self.tcr.accountServerConstants.getString('priceFirstMonth'), text_scale=priceTextScale, text_fg=priceColor, text_shadow=(0, 0, 0, 1), text_shadowOffset=priceShadowOffset)
        self.pricePerMonth = DirectLabel(parent=self.pricingLabel, relief=None, pos=(0.455, 0, 0), text='$%s' % self.tcr.accountServerConstants.getString('pricePerMonth'), text_scale=priceTextScale, text_fg=priceColor, text_shadow=(0, 0, 0, 1), text_shadowOffset=priceShadowOffset)
        linePos -= lineHeight
        if self.showLegalBlurb:
            self.agreementLabel = DirectLabel(parent=self.frame, relief=None, pos=(-1.2, 0, linePos + lineHeight * 0.1), text=Localizer.BillingScreenAgreementText, text_scale=0.052, text_align=TextNode.ALeft, text_fg=(0.1, 0.2, 0.1, 1), text_wordwrap=42)
            linePos -= lineHeight
        textScale, entryScale, lineHeight, buttonScale, buttonLineHeight, entryWidth, entryFrameSize = oldValues
        linePos = -0.8
        self.submitButton = DirectButton(parent=self.frame, relief=None, pos=(0, 0, linePos), scale=buttonScale, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), text=Localizer.BillingScreenSubmit, text_scale=0.06, text_pos=(0, -0.02), image_scale=(1.3, 1.1, 1.1), command=self.__handleSubmit)
        linePos -= buttonLineHeight
        self.cancelButton = DirectButton(parent=self.frame, relief=None, pos=(0, 0, linePos), scale=buttonScale, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1.3, 1.1, 1.1), image0_color=Vec4(1, 0.1, 0.1, 1), image1_color=Vec4(1, 0.1, 0.1, 1), image2_color=Vec4(1, 1, 1, 1), text=Localizer.BillingScreenCancel, text_scale=0.06, text_pos=(0, -0.02), command=self.__handleCancel)
        linePos -= buttonLineHeight
        ...
        self.whySafeDialog = ToontownDialog.GlobalDialog(dialogName='whySafeDialog', okButtonText=Localizer.BillingScreenWhySafeClose, doneEvent=self.whySafeDoneEvent, style=ToontownDialog.Acknowledge, text=Localizer.BillingScreenWhySafeText, text_scale=0.05, text_wordwrap=48, pos=(0, 0, 0.12), scale=0.9)
        self.whySafeDialog.hide()
        self.whySafeTitle = DirectLabel(parent=self.whySafeDialog, relief=None, pos=(-0.78, 0, 0.71), scale=0.075, text=Localizer.BillingScreenWhySafeTitle, text_font=getMinnieFont(), text_fg=(0, 0, 1, 1))
        self.whySafeCCGuarantee = DirectLabel(parent=self.whySafeDialog, relief=None, pos=(-0.61, 0, 0.02), scale=0.075, text=Localizer.BillingScreenWhySafeCreditCardGuarantee, text_font=getMinnieFont(), text_fg=(1, 0.5, 0.1, 1))
        self.whySafeJoin = DirectLabel(parent=self.whySafeDialog, relief=None, pos=(-0.81, 0, -0.78), scale=0.075, text=Localizer.BillingScreenWhySafeJoin, text_font=getMinnieFont(), text_fg=(1, 0, 0, 1))
        self.whySafeToontown = DirectLabel(parent=self.whySafeDialog, relief=None, pos=(0, 0, -0.78), scale=0.075, text=Localizer.BillingScreenWhySafeToontown, text_font=getMinnieFont(), text_fg=(1, 0.5, 0.1, 1))
        self.whySafeToday = DirectLabel(parent=self.whySafeDialog, relief=None, pos=(0.86, 0, -0.78), scale=0.075, text=Localizer.BillingScreenWhySafeToday, text_font=getMinnieFont(), text_fg=(1, 0, 0, 1))
        self.whySafeButton = DirectButton(parent=self.frame, relief=None, pos=(-1.1, 0, -0.94), image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1, 0.8, 0.8), text=Localizer.BillingScreenWhySafe, text_pos=(0, -0.01), text_scale=0.042, command=self.showWhySafe)
        self.privacyPolicyButton = DirectButton(parent=self.frame, relief=None, pos=(1.1, 0, -0.94), scale=1.1, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1, 0.8, 0.8), text=Localizer.BillingScreenPrivacyPolicy, text_pos=(0, -0.012), text_scale=0.054, command=self.showPrivacyPolicy)
        self.pleaseWaitDialog = DirectFrame(relief=None, image=getDefaultDialogGeom(), image_color=GlobalDialogColor, image_scale=(1, 1, 0.5), text=Localizer.BillingScreenPleaseWait, text_scale=0.1, text_pos=(0, -0.03), sortOrder=NO_FADE_SORT_INDEX)
    ...
    def enterCreate(self):
        ...
        self.__showStateOrProvince()
        self.countryControl['command'] = self.__handleCountry
        self.ccTypeControl['command'] = self.__handleCCType
        self.ccYearControl['command'] = self.__handleCCYear
    ...
    def restartFocusMgmt(self):
        ...
        self.stopFocusMgmt()
        self.focusList = [
         self.emailEntry, self.emailConfirmEntry, self.address1Entry, self.address2Entry, self.cityEntry, 'country', 'stateProvince', self.zipCodeEntry, 'ccType', self.ccNumberEntry, 'ccExpDate', self.nameOnCardEntry]
        if not self.handleControlFocus:
            ...
            controls = {'country': self.countryControl, 'stateProvince': None, 'ccType': self.ccTypeControl, 'ccExpDate': self.ccMonthControl}
            ...
            if countryIndex == self.countryCodes.index('US'):
                ...
                if countryIndex == self.countryCodes.index('CA'):
        ...
        self.startFocusMgmt(startFocus=curFocusIndex, overrides={self.address2Entry: GuiScreen.GuiScreen.ENTERPRESS_ADVANCE, self.provinceEntry: GuiScreen.GuiScreen.ENTERPRESS_ADVANCE, self.ccTypeControl: self.__handleCCTypeEnterPress, self.nameOnCardEntry: GuiScreen.GuiScreen.ENTERPRESS_DONT_ADVANCE}, globalFocusHandler=self.__handleFocusChange)
        return
    ...
    def enterWaitForLoginResponse(self):
        ...
        try:
            try:
                ...
                argDict = {'email': self.email, 'ccType': self.ccType, 'ccNumber': self.ccNumber, 'ccMonth': self.ccMonth, 'ccYear': self.ccYear, 'nameOnCard': self.nameOnCard, 'addr1': self.address1, 'addr2': self.address2, 'city': self.city, 'country': self.countryCode, 'zip': self.zipCode}
                if self.countryCode == 'US':
            ...
            if error:
                ...
                self.dialog.show()
                self.acceptOnce(self.dialogDoneEvent, self.__handleErrorAck)
    ...
    def __handleCCYear(self):
        self.setFocus(self.ccMonthControl)

    def __handleCCType(self):
    ...
    def __handleCCTypeEnterPress(self):
        self.advanceFocus(self.removedCCTypePleaseChoose)

    def __handleCountry(self):
        defaultCountry = 'US'
        longestCountry = 'VC'
    ...
    def __showStateOrProvince(self):
        ...
        if self.countryControl.getSelectedIndex() == self.countryCodes.index('US'):
            ...
            if self.countryControl.getSelectedIndex() == self.countryCodes.index('CA'):
    ...
    def __handleNameOnCard(self, name):
        if name != '':
            self.__handleSubmit()
    ...
    def __handleSubmit(self):
        ...
        self.origInputs = {'ccNumber': self.ccNumber}
        ...
        checks = (
         (
          self.email, [Localizer.BillingScreenEnterEmail, self.emailEntry]), ([InputCheck.isValidEmailAddr, [self.email]], [Localizer.BillingScreenEnterValidEmail, self.emailEntry]), (self.confirmEmail, [Localizer.BillingScreenEnterEmailConfirm, self.emailConfirmEntry]), ([InputCheck.emailAddrMatch, [self.email, self.confirmEmail]], [handleEmailMismatch, []]), (self.address1 + self.address2, [Localizer.BillingScreenEnterAddress, self.address1Entry]), (self.city, [Localizer.BillingScreenEnterAddress, self.cityEntry]), (self.zipCode, [Localizer.BillingScreenEnterAddress, self.zipCodeEntry]), ([isValidCreditCardType, [self.ccType]], [Localizer.BillingScreenChooseCreditCardType, self.ccTypeControl]), (self.ccNumber, [Localizer.BillingScreenEnterCreditCardNumber, self.ccNumberEntry]), ([InputCheck.isValidCreditCardNum, [self.ccNumber]], [Localizer.BillingScreenEnterValidCreditCardNumber, self.ccNumberEntry]), ([InputCheck.isValidCreditCardNum, [self.ccNumber, self.ccType]], [Localizer.BillingScreenEnterValidSpecificCreditCardNumber % self.ccType, self.ccNumberEntry]), ([InputCheck.isValidCreditCardExpDate, [self.ccMonth, self.ccYear, self.tcr.dateObject.getMonth(), self.tcr.dateObject.getYear()]], [Localizer.BillingScreenEnterValidCreditCardExpDate, self.ccMonthControl]), (self.nameOnCard, [Localizer.BillingScreenEnterNameOnCard, self.nameOnCardEntry]))
    ...
    def __handleCancel(self):
        ...
        self.acceptOnce('confirmCancelDone', self.__handleConfirmCancel)

    def __handleConfirmCancel(self):
