...
import MovieCamera, DirectNotifyGlobal, MovieUtil
notify = DirectNotifyGlobal.directNotify.newCategory('MovieDrop')
hitSoundFiles = (
 'AA_drop_flowerpot.mp3', 'AA_drop_sandbag.mp3', 'AA_drop_anvil.mp3', 'AA_drop_bigweight.mp3', 'AA_drop_safe.mp3', 'AA_drop_piano.mp3')
missSoundFiles = (
 'AA_drop_flowerpot_miss.mp3', 'AA_drop_sandbag_miss.mp3', 'AA_drop_anvil_miss.mp3', 'AA_drop_bigweight_miss.mp3', 'AA_drop_safe_miss.mp3', 'AA_drop_piano_miss.mp3')
tDropShadow = 1.3
...
dPropFall = 0.6
objects = (
 'flowerpot', 'sandbag', 'anvil', 'weight', 'safe', 'piano')
objZOffsets = (
...
def __dropObject(drop, delay, objName, level, alreadyDodged, alreadyTeased):
    ...
    toonIntervals.append(FunctionInterval(toon.setHpr, extraArgs=[battle, origHpr]))
    toonTrack = Track(toonIntervals)
    buttonIntervals = []
    buttonShow = FunctionInterval(MovieUtil.showProps, extraArgs=[buttons, hands])
    ...
    objIntervals.append(FunctionInterval(battle.movie.needRestoreRenderProp, extraArgs=[object]))
    objInit = FunctionInterval(posObject, extraArgs=[object, suit, level, majorObject, hp <= 0])
    ...
    if hp > 0 or level == 1 or level == 2:
        ...
        poseProp = FunctionInterval(poseProp, extraArgs=[object, objName, level])
    ...
    objIntervals.append(FunctionInterval(MovieUtil.removeProp, extraArgs=[object]))
    objIntervals.append(FunctionInterval(battle.movie.clearRenderProp, extraArgs=[object]))
    ...
    shadowTrack = Track([WaitInterval(delay + tButtonPressed), FunctionInterval(battle.movie.needRestoreRenderProp, extraArgs=[dropShadow]), FunctionInterval(posShadow),
     LerpScaleInterval(dropShadow, tObjectAppears - tButtonPressed, dropShadow.getScale(), startScale=Point3(0.01, 0.01, 0.01)), WaitInterval(0.3), FunctionInterval(MovieUtil.removeProp, extraArgs=[dropShadow]), FunctionInterval(battle.movie.clearRenderProp, extraArgs=[dropShadow])])
    if hp > 0:
        suitIntervals = []
        ...
        bonusTrack = None
        if hpbonus > 0:
            bonusTrack = Track([(delay + tObjectAppears + 0.75, FunctionInterval(suit.showLaffNumber, openEnded=0, extraArgs=[-hpbonus, 1]))])
        if died != 0:
        ...
        if kbbonus == 0:
            suitTrack = Track([(delay + tObjectAppears, FunctionInterval(MovieUtil.indicateMissed, extraArgs=[suit, 0.6])), FunctionInterval(suit.loop, extraArgs=['neutral'])])
        else:
            if alreadyDodged > 0:
            ...
            if level >= 3:
                if alreadyTeased > 0:
                    ...
                    suitTrack = Track([MovieUtil.createSuitTeaseMultiTrack(suit, delay=delay + tObjectAppears)])
            else:
