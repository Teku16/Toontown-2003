import Avatar, AvatarDNA, Actor, string
from ToonHead import *
from PandaModules import *
from IntervalGlobal import *
import DirectNotifyGlobal, ToontownGlobals, Localizer, random, Wake, Emote, Motion
...
Phase5AnimList = (
 (
  'water-gun', 'water-gun'), ('hold-bottle', 'hold-bottle'), ('firehose', 'firehose'), ('spit', 'spit'), ('tickle', 'tickle'), ('smooch', 'smooch'), ('happy-dance', 'happy-dance'), ('sprinkle-dust', 'sprinkle-dust'), ('juggle', 'juggle'), ('sound', 'shout'), ('toss', 'toss'), ('hold-magnet', 'hold-magnet'), ('hypnotize', 'hypnotize'), ('struggle', 'struggle'), ('lose', 'lose'), ('think', 'think'), ('melt', 'melt'))
...
toonBodyScales = {'mouse': 0.6, 'cat': 0.73, 'fowl': 0.66, 'rabbit': 0.74, 'horse': 0.85, 'dog': 0.85}
toonHeadScales = {'mouse': Point3(1.3078, 1.1274, 1.1274), 'cat': Point3(1.125), 'fowl': Point3(1.345, 1.1298, 1.3292), 'rabbit': Point3(1.0), 'horse': Point3(1.0), 'dog': Point3(1.0857)}
legHeightDict = {'s': 1.5, 'm': 2.0, 'l': 2.75}
torsoHeightDict = {'s': 1.5, 'm': 1.75, 'l': 2.25, 'ss': 1.5, 'ms': 1.75, 'ls': 2.25, 'sd': 1.5, 'md': 1.75, 'ld': 2.25}
headHeightDict = {'dls': 0.75, 'dss': 0.5, 'dsl': 0.5, 'dll': 0.75, 'cls': 0.75, 'css': 0.5, 'csl': 0.5, 'cll': 0.75, 'hls': 0.75, 'hss': 0.5, 'hsl': 0.5, 'hll': 0.75, 'mls': 0.75, 'mss': 0.5, 'rls': 0.75, 'rss': 0.5, 'rsl': 0.5, 'rll': 0.75, 'fls': 0.75, 'fss': 0.5, 'fsl': 0.5, 'fll': 0.75}
            ...
            if loadFlag:
                ...
                if LegsAnimDict[key].has_key(anim[0]):
                    if toonbase.localToon.style.legs == key:
                        toonbase.localToon.unloadAnims([anim[0]], 'legs', None)
            ...
            if loadFlag:
                ...
                if TorsoAnimDict[key].has_key(anim[0]):
                    if toonbase.localToon.style.torso == key:
                        toonbase.localToon.unloadAnims([anim[0]], 'torso', None)
                ...
                if loadFlag:
                    ...
                    if HeadAnimDict[key].has_key(anim[0]):
                        if toonbase.localToon.style.head == key:
                            toonbase.localToon.unloadAnims([anim[0]], 'head', None)
...
class Toon(Avatar.Avatar, ToonHead):
    ...
    def setupToonNodes(self):
        ...
        if not launcher or launcher and launcher.getPhaseComplete(3.5):
            ...
            self.bookActors = [bookActor, bookActor2, bookActor3]
            ...
            self.holeActors = [holeActor, holeActor2, holeActor3]
        else:
            self.bookActors = []
            self.holeActors = []
        ...
        if self.hasLOD():
            if self.style.getGender() == 'f' and fromNet == 0:
                ...
                if self.style.torso[1] == 's' and bottomPair[1] == AvatarDNA.SKIRT:
                    ...
                    if self.style.torso[1] == 'd' and bottomPair[1] == AvatarDNA.SHORTS:
    ...
    def findSomethingToLookAt(self):
        if whrandom.random() < 0.1 or not hasattr(self, 'cr'):
    ...
    def setSpeed(self, forwardSpeed, rotateSpeed):
        ...
        if self.standWalkRunReverse != None:
            ...
            try:
                ...
                if id == zoneId == ToontownGlobals.DonaldsDock:
                    wakeWaterHeight = ToontownGlobals.DDWakeWaterHeight
                    showWake = 1
                else:
                    if id == zoneId == ToontownGlobals.ToontownCentral:
                        wakeWaterHeight = ToontownGlobals.TTWakeWaterHeight
                        showWake = 1
                    else:
                        if id == ToontownGlobals.MyEstate:
    ...
    def enterTeleportOut(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        holeTrack = Track((0.0, Func(showHoles, holes, hands)), (
         0.5, SoundInterval(self.getSoundTeleport())), (
        ...
        for hole in holes:
            self.track.append(ActorInterval(hole, 'hole', duration=3.4))

        self.track.append(ActorInterval(self, 'teleport', duration=3.4))
    ...
    def enterEmote(self, animMultiplier=1, ts=0, callback=None, extraArgs=[]):
        ...
        if emoteIndex != Emote.EmoteSleepIndex:
            taskMgr.doMethodLater(duration, self.__finishEmote, self.taskName('finishEmote'))
        return None
        return
    ...
    def __doToonColorScale(self, scale, lerpTime):
        if scale == None:
                                ...
                                if effect == ToontownGlobals.CEFlatPortrait:
                                    ...
                                    if effect == ToontownGlobals.CEFlatProfile:
                                        ...
                                        if effect == ToontownGlobals.CETransparent:
                                            return self.__doToonColorScale(VBase4(1, 1, 1, 0.6), lerpTime)
                                        else:
                    ...
                    if effect == ToontownGlobals.CESmallToon:
                        ...
                        if effect == ToontownGlobals.CEFlatPortrait:
                            ...
                            if effect == ToontownGlobals.CEFlatProfile:
                                ...
                                if effect == ToontownGlobals.CETransparent:
                                    return self.__doToonColorScale(None, lerpTime)
                                else:
                                    if effect == ToontownGlobals.CENoColor:
