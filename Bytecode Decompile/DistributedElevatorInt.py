...
import DistributedObject, DirectNotifyGlobal, FSM, State, NodePath, CollisionSphere, CollisionNode, ZoneUtil, DelayDelete
...
class DistributedElevatorInt(DistributedObject.DistributedObject):
    ...
    def forcedExit(self, avId):
        ...
        toonbase.tcr.playGame.getPlace().fsm.request('teleportOut', [{'loader': ZoneUtil.getLoaderName(target_sz), 'where': ZoneUtil.getWhereName(target_sz, 1), 'how': 'teleportIn', 'hoodId': target_sz, 'zoneId': target_sz, 'shardId': None, 'avId': -1}])
        return
