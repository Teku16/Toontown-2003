...
import QuestPoster, ToontownTimer, ToontownGlobals, Localizer

class QuestChoiceGui(DirectFrame):
    __module__ = __name__

    def __init__(self):
        DirectFrame.__init__(self, relief=None, geom=getDefaultDialogGeom(), geom_color=Vec4(0.8, 0.6, 0.4, 1), geom_scale=(1.85, 1, 0.9), geom_hpr=(0, 0, 90), pos=(-0.85, 0, 0))
        self.initialiseoptions(QuestChoiceGui)
        self.questChoicePosters = []
        guiButton = loader.loadModelOnce('phase_3/models/gui/quit_button')
        self.cancelButton = DirectButton(parent=self, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text=Localizer.QuestChoiceGuiCancel, text_scale=0.06, text_pos=(0, -0.02), command=self.chooseQuest, extraArgs=[0])
        guiButton.removeNode()
        self.timer = ToontownTimer.ToontownTimer()
        self.timer.reparentTo(self)
        self.timer.setScale(0.3)
        toonbase.setCellsAvailable(toonbase.leftCells, 0)
        toonbase.setCellsAvailable([toonbase.bottomCells[0], toonbase.bottomCells[1]], 0)
        return
    ...
    def chooseQuest(self, questId):
        ...
        toonbase.setCellsAvailable([toonbase.bottomCells[0], toonbase.bottomCells[1]], 1)
        self.timer.stop()
