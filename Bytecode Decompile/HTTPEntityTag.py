...
class HTTPEntityTag(FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_constructor_ptrConstHTTPEntityTag(self, copy):
        self.this = libpandaexpress._inP2KOdRDIw(copy.this)
        self.userManagesMemory = 1

    def __overloaded_constructor_atomicstring(self, text):
        self.this = libpandaexpress._inP2KOd6Xv8(text)
        self.userManagesMemory = 1

    def __overloaded_constructor_bool_atomicstring(self, weak, tag):
            ...
            if numArgs == 1:
                if isinstance(_args[0], types.StringType):
                    ...
                    if isinstance(_args[0], HTTPEntityTag):
                        return self.__overloaded_constructor_ptrConstHTTPEntityTag(_args[0])
                    else:
                ...
                if numArgs == 2:
                    if isinstance(_args[0], types.IntType):
                        if isinstance(_args[1], types.StringType):
                            return self.__overloaded_constructor_bool_atomicstring(_args[0], _args[1])
                        else:
