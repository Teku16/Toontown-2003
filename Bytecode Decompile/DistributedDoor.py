...
import ToontownGlobals, DirectNotifyGlobal, FSM, DistributedObject, ZoneUtil, Suit, DelayDelete, FADoorCodes, DoorTypes, ToontownDialog
...
class DistributedDoor(DistributedObject.DistributedObject):
        ...
        if not self.__dict__.has_key('building'):
            if self.doorType == DoorTypes.INT_STANDARD:
                ...
                if self.doorType == DoorTypes.INT_HQ:
                    ...
                    if self.doorType == DoorTypes.EXT_STANDARD or self.doorType == DoorTypes.EXT_HQ:
    ...
    def enterClosing(self, ts):
        ...
        if hasattr(self, 'done'):
            ...
            request = {'loader': ZoneUtil.getBranchLoaderName(zoneId), 'where': ZoneUtil.getToonWhereName(zoneId), 'how': 'doorIn', 'hoodId': ZoneUtil.getHoodId(zoneId), 'zoneId': zoneId, 'shardId': None, 'avId': -1, 'doorDoId': self.otherDoId}
    ...
    def enterOpening(self, ts):
        ...
        self.doorTrack = Parallel(SoundInterval(self.openSfx, node=rightDoor), Sequence(HprInterval(rightDoor, VBase3(0, 0, 0), other=otherNP), Wait(0.4), Func(rightDoor.show), Func(doorFrameHoleRight.show), LerpHprInterval(node=rightDoor, duration=0.6, hpr=VBase3(100, 0, 0), startHpr=VBase3(0, 0, 0), other=otherNP, blendType='easeInOut')), name=trackName)
        self.doorTrack.start(ts)
    ...
    def exitDoorEnterOpening(self, ts):
        ...
        if not leftDoor.isEmpty():
            ...
            self.doorExitTrack = Parallel(SoundInterval(self.openSfx, node=leftDoor), Sequence(Func(leftDoor.show), Func(doorFrameHoleLeft.show), LerpHprInterval(node=leftDoor, duration=0.6, hpr=VBase3(-100, 0, 0), startHpr=VBase3(0, 0, 0), other=otherNP, blendType='easeInOut')), name=trackName)
            self.doorExitTrack.start(ts)
        else:
