...
class LineParticleRenderer(BaseParticleRenderer.BaseParticleRenderer, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_constructor_ptrConstLVecBase4f_ptrConstLVecBase4f___enum__ParticleRendererAlphaMode(self, head, tail, alphaMode):
        self.this = libpandaphysics._inPKBUArY3q(head.this, tail.this, alphaMode)
        self.userManagesMemory = 1

    def __overloaded_constructor_ptrConstLineParticleRenderer(self, copy):
        ...
        if numArgs == 0:
            ...
            if numArgs == 1:
                if isinstance(_args[0], LineParticleRenderer):
                    return self.__overloaded_constructor_ptrConstLineParticleRenderer(_args[0])
                else:
                    ...
                    if isinstance(_args[0], VBase4.VBase4):
                        ...
                        if isinstance(_args[1], VBase4.VBase4):
                            if isinstance(_args[2], types.IntType):
                                return self.__overloaded_constructor_ptrConstLVecBase4f_ptrConstLVecBase4f___enum__ParticleRendererAlphaMode(_args[0], _args[1], _args[2])
                            else:
