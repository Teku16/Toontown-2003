...
import FSM, State, ToontownGlobals, ToontownTimer, NodePath, Task, Trajectory, math, ToonHead, Splash, DustCloud, CannonGameGlobals
...
class DistributedCannonGame(DistributedMinigame):
    ...
    def load(self):
        ...
        self.rewardPanelTitle = DirectLabel(parent=self.rewardPanel, relief=None, pos=(0, 0, 0.06), scale=0.08, text=Localizer.CannonGameReward, text_fg=(0.95, 0.95, 0, 1), text_shadow=(0, 0, 0, 1))
    ...
    def __createCannons(self):
        ...
        for i in range(numAvs):
            ...
            self.cannonPositionDict[avId] = [0, CANNON_ANGLE_MIN]
    ...
    def __fireCannonTask(self, task):
        ...
        shootTask = Task.Task(self.__shootTask)
        flyTask = Task.Task(self.__flyTask)
        seqDoneTask = Task.Task(self.__flySequenceDoneTask)
        info = {}
        info['avId'] = avId
    ...
    def __startIntro(self):
        ...
        taskLookInWater = Task.Task(self.__taskLookInWater)
        taskPullBackFromWater = Task.Task(self.__taskPullBackFromWater)
        taskFlyUpToToon = Task.Task(self.__flyUpToToon)
        taskFlyToBackOfCannon = Task.Task(self.__flyToBackOfCannon)
        commonData = {}
