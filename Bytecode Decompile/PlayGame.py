...
import DirectNotifyGlobal, PandaObject, StateData, FSM, State
from ToontownMsgTypes import *
from ToontownGlobals import *
import ToontownGlobals, TTHood, DDHood, MMHood, BRHood, DGHood, DLHood, TutorialHood, TaskManagerGlobal, QuietZoneState, ZoneUtil, Estate, EstateLoader, Localizer
...
class PlayGame(PandaObject.PandaObject, StateData.StateData):
    ...
    def __init__(self, parentFSM, doneEvent):
        ...
        self.fsm = FSM.FSM('PlayGame', [
         ...
         State.State('quietZone', self.enterQuietZone, self.exitQuietZone, [
          'TTHood', 'DDHood', 'BRHood', 'MMHood', 'DGHood', 'DLHood', 'TutorialHood', 'EstateLoader']),
    ...
    def getEstateZoneAndGoHome(self, avId, zoneId):
        self.doneStatus = {'avId': avId, 'zoneId': zoneId, 'hoodId': MyEstate, 'loader': 'estateLoader', 'how': 'teleportIn', 'shardId': None}
    ...
    def getHoodClassByNumber(self, hoodNumber):
        if hoodNumber == ToontownCentral:
            ...
            if hoodNumber == DonaldsDock:
                return DDHood.DDHood
            else:
                if hoodNumber == TheBrrrgh:
                    return BRHood.BRHood
                else:
                    if hoodNumber == MinniesMelodyland:
                        return MMHood.MMHood
                    else:
                        if hoodNumber == DaisyGardens:
                            return DGHood.DGHood
                        else:
                            if hoodNumber == DonaldsDreamland:
                                return DLHood.DLHood
                            else:
                                if hoodNumber == Tutorial:
                                    return TutorialHood.TutorialHood
                                else:
                                    if hoodNumber == MyEstate:
