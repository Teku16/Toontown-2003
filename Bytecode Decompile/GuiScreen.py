...
import ToontownDialog, DirectNotifyGlobal
...
class GuiScreen:
    ...
    def startFocusMgmt(self, startFocus=0, enterPressBehavior=ENTERPRESS_ADVANCE_IFNOTEMPTY, overrides={}, globalFocusHandler=None):
        ...
        for i in xrange(len(self.focusList)):
            ...
            if callable(behavior):
                ...
                if not isinstance(item, DirectEntry) and behavior == GuiScreen.ENTERPRESS_ADVANCE_IFNOTEMPTY:
    ...
    def waitForDatabaseTimeout(self):
        globalClock.tick()
        taskMgr.doMethodLater(DatabaseDialogTimeout, self.__showWaitingForDatabase, 'waitingForDatabase')

    def __showWaitingForDatabase(self, task):
        self.waitingForDatabase = ToontownDialog.ToontownDialog(text=Localizer.GuiScreenToontownUnavailable, dialogName='WaitingForDatabase', buttonTextList=['Cancel'], style=ToontownDialog.Acknowledge, command=self.__handleCancelWaiting)
