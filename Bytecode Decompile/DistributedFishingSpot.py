...
import DistributedObject, DirectNotifyGlobal, ToontownGlobals, FishingCodes, FishPage, Localizer, Quests, Actor, Rope, math
...
class DistributedFishingSpot(DistributedObject.DistributedObject):
    ...
    def setOccupied(self, avId):
        ...
        if self.avId == 0:
            ...
            if self.avId == toonbase.localToon.doId:
                ...
                toonbase.setCellsAvailable([toonbase.bottomCells[1], toonbase.bottomCells[2]], 0)
        ...
        if wasLocalToon and not self.localToonFishing:
            self.__hideGui()
            toonbase.setCellsAvailable([toonbase.bottomCells[1], toonbase.bottomCells[2]], 1)
        ...
        if mode == FishingCodes.NoMovie:
            ...
            if mode == FishingCodes.EnterMovie:
                ...
                if self.localToonFishing:
                    trackList.append(LerpPosHprInterval(node=camera, other=self.av, duration=1.5, pos=Point3(14, -7.4, 7.3), hpr=VBase3(45, -12, 0), blendType='easeInOut'))
                self.av.disableBlend()
                                    ...
                                    if mode == FishingCodes.PullInMovie:
                                        ...
                                        if self.localToonFishing:
                                            ...
                                            if code == FishingCodes.QuestItem:
                                                ...
                                                if code == FishingCodes.FishItem:
                                                    self.__showFishItem(item)
                                                else:
                                                    if code == FishingCodes.OverLimitFishItem:
                                                        self.__hideGui()
    ...
    def __doNibbleBob(self, task):
        ...
        z = math.sin(now) * 0.05 + nibbleEffect
        self.bob.setZ(self.bobSpot[2] + z)
        return Task.cont
    ...
    def __showReelGui(self):
        ...
        self.crankHandle.bind(B1PRESS, self.__clickCrank)
        self.crankHandle.bind(B1RELEASE, self.__releaseCrank)
    ...
    def __updateSpeedGauge(self):
        ...
        if totalTime > 0:
            ...
            if pctDiff >= FishingCodes.ManualReelMatch * 0.8:
                self.tooFast.show()
            else:
                if pctDiff <= -FishingCodes.ManualReelMatch * 0.8:
                    self.tooSlow.show()
