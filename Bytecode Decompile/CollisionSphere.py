...
class CollisionSphere(CollisionSolid.CollisionSolid, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_constructor_float_float_float_float(self, cx, cy, cz, radius):
    ...
    def __overloaded_setCenter_ptrCollisionSphere_ptrConstLPoint3f(self, center):
        returnValue = libpanda._inPHwca0CUl(self.this, center.this)
        return returnValue

    def __overloaded_setCenter_ptrCollisionSphere_float_float_float(self, x, y, z):
    ...
    def setCenter(self, *_args):
        ...
        if numArgs == 1:
            import Point3
            if isinstance(_args[0], Point3.Point3):
                return self.__overloaded_setCenter_ptrCollisionSphere_ptrConstLPoint3f(_args[0])
            else:
                raise TypeError, 'Invalid argument 0, expected one of: <Point3.Point3> '
        else:
            if numArgs == 3:
                if isinstance(_args[0], types.FloatType) or isinstance(_args[0], types.IntType):
                    if isinstance(_args[1], types.FloatType) or isinstance(_args[1], types.IntType):
                        if isinstance(_args[2], types.FloatType) or isinstance(_args[2], types.IntType):
                            return self.__overloaded_setCenter_ptrCollisionSphere_float_float_float(_args[0], _args[1], _args[2])
                        else:
                            raise TypeError, 'Invalid argument 2, expected one of: <types.FloatType> '
                    else:
                        raise TypeError, 'Invalid argument 1, expected one of: <types.FloatType> '
