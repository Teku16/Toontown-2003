...
import DirectNotifyGlobal, StateData, FSM, State, Task, Purchase, OnscreenText, DistributedAvatar, SuitInterior, QuietZoneState, ZoneUtil, Localizer
...
class Hood(StateData.StateData):
    ...
    def spawnTitleText(self, zoneId):
        ...
        seq = Task.sequence(Task.pause(0.1), Task.pause(6.0), self.titleText.lerpColor(Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], self.titleColor[3]), Vec4(self.titleColor[0], self.titleColor[1], self.titleColor[2], 0.0), 0.5), Task.Task(self.hideTitleTextTask))
    ...
    def isSameHood(self, status):
        return status['hoodId'] == self.id and status['shardId'] == None
        return
    ...
    def handlePurchaseDone(self):
        ...
        if doneStatus['where'] == 'playground':
            if self.id == Tutorial:
                newDoneStatus = {'loader': 'safeZoneLoader', 'where': 'playground', 'how': 'teleportIn', 'hoodId': ToontownCentral, 'zoneId': ToontownCentral, 'shardId': None, 'avId': -1}
                self.doneStatus = newDoneStatus
                messenger.send(self.doneEvent)
            else:
                self.fsm.request('quietZone', [{'loader': 'safeZoneLoader', 'where': 'playground', 'how': 'teleportIn', 'hoodId': self.id, 'zoneId': self.id, 'shardId': None, 'avId': -1}])
        else:
