...
import DirectNotifyGlobal, PandaObject, StateData, FSM, State, Emote
...
class Walk(PandaObject.PandaObject, StateData.StateData):
    ...
    def __init__(self, doneEvent):
        ...
        self.fsm = FSM.FSM('Walk', [
         State.State('off', self.enterOff, self.exitOff, [
          'walking', 'swimming', 'slowWalking', 'jumping']),
         State.State('walking', self.enterWalking, self.exitWalking, [
          'swimming', 'slowWalking', 'jumping']),
    ...
    def enter(self, slowWalk=0):
        ...
        self.accept('control', self.__handleJumpKey)
    ...
    def __swim(self, task):
        ...
        if speed == 0 and self.swimSoundPlaying:
            ...
            if speed > 0 and self.swimSoundPlaying == 0 and self.IsSwimSoundAudible:
                self.swimSoundPlaying = 1
                base.playSfx(self.swimSound, looping=1)
        return Task.cont

    def enterSlowWalking(self):
        self.accept(toonbase.localToon.uniqueName('positiveHP'), self.__handlePositiveHP)
        toonbase.localToon.startTrackAnimToSpeed()
