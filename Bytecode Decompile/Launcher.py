...
import libdtoolconfig, libpandaexpress
...
class Launcher(DirectObject):
    ...
    def download(self, serverFilePath, localFilename, callback):
        self.launcherMessage(Localizer.LauncherDownloadFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
    ...
    def downloadTask(self, task):
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                if totalBytes:
                    ...
                    self.launcherMessage(Localizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': pct})
                else:
                    self.launcherMessage(Localizer.LauncherDownloadFileBytes % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'bytes': bytesWritten})
        ...
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            ...
            if self.httpChannel.isValid():
                ...
                if int(self.httpChannel.getStatusCode() / 100) == 2:
    ...
    def downloadMultifileTask(self, task):
        if self.httpChannel.run():
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(Localizer.LauncherDownloadFilePercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': percentComplete})
            self.foregroundSleep()
            return Task.cont
        if self.httpChannel.isValid() and self.httpChannel.isDownloadComplete():
            self.downloadMultifileWriteToDisk(task)
    ...
    def decompressFile(self, localFilename, callback):
        ...
        self.launcherMessage(Localizer.LauncherDecompressingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
    ...
    def decompressFileTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(Localizer.LauncherDecompressingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100))})
            self.foregroundSleep()
            return Task.cont
        else:
            if errorCode == EUSuccess:
                self.launcherMessage(Localizer.LauncherDecompressingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
    ...
    def extract(self, mfname, localFilename, destDir, callback):
        ...
        self.launcherMessage(Localizer.LauncherExtractingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
    ...
    def extractTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(Localizer.LauncherExtractingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100.0))})
            ...
            if errorCode == EUSuccess:
                self.launcherMessage(Localizer.LauncherExtractingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
    ...
    def patch(self, patchFile, patcheeFile, callback):
        ...
        self.launcherMessage(Localizer.LauncherPatchingFile % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
    ...
    def patchTask(self, task):
        ...
        if errorCode == EUOk:
            ...
            if now - task.lastUpdate >= self.UserUpdateDelay:
                ...
                self.launcherMessage(Localizer.LauncherPatchingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': int(round(progress * 100.0))})
            self.foregroundSleep()
            return Task.cont
        else:
            if errorCode == EUSuccess:
                self.launcherMessage(Localizer.LauncherPatchingPercent % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases, 'percent': 100})
    ...
    def patchMultifile(self):
        self.launcherMessage(Localizer.LauncherCheckUpdates % {'name': self.currentPhaseName, 'current': self.currentPhaseIndex, 'total': self.numPhases})
