...
soundFiles = (
 'AA_sound_bikehorn.mp3', 'AA_sound_whistle.mp3', 'AA_sound_bugle.mp3', 'AA_sound_aoogah.mp3', 'AA_sound_elephant.mp3', 'SZ_DD_foghorn.mp3')
...
def __doSoundsLevel(sounds, delay, hitCount):
    ...
    for sound in sounds:
        ...
        megaphoneIntervals = []
        megaphoneShow = FunctionInterval(MovieUtil.showProps, extraArgs=[megaphones, hands])
        ...
        megaphoneHide = FunctionInterval(MovieUtil.removeProps, extraArgs=[megaphones])
        ...
        for target in targets:
            ...
            if totalDamage > 0 and sound == lastSoundThatHit:
                ...
                suitIntervals = []
                showDamage = FunctionInterval(suit.showLaffNumber, openEnded=0, extraArgs=[-totalDamage])
                updateHealthBar = FunctionInterval(suit.updateHealthBar, extraArgs=[totalDamage])
                ...
                bonusTrack = None
                if hpbonus > 0:
                    bonusTrack = Track([(delay + tSuitReact + 0.75, FunctionInterval(suit.showLaffNumber, openEnded=0, extraArgs=[-hpbonus, 1]))])
                if died != 0:
                ...
                if bonusTrack == None:
                    ...
                    tracks.append(MultiTrack([Track(suitIntervals), bonusTrack]))
            else:
                if totalDamage <= 0:
                    tracks.append(Track([(2.9, FunctionInterval(MovieUtil.indicateMissed, extraArgs=[suit, 1.0]))]))
