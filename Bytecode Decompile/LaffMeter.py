...
import DistributedAvatar, ToontownGlobals
...
class LaffMeter(DirectFrame):
    ...
    def animatedEffect(self, delta):
        ...
        if delta > 0:
            Sequence(self.container.scaleInterval(0.2, 1.333, blendType='easeOut'), self.container.scaleInterval(0.2, 1, blendType='easeIn'), name=taskName, autoFinish=1).play()
        else:
            Sequence(self.container.scaleInterval(0.2, 0.666, blendType='easeOut'), self.container.scaleInterval(0.2, 1, blendType='easeIn'), name=taskName, autoFinish=1).play()
        return
