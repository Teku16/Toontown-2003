...
import ParticleSystem, BaseParticleFactory, PointParticleFactory, ZSpinParticleFactory, BaseParticleRenderer, PointParticleRenderer, LineParticleRenderer, GeomParticleRenderer, SparkleParticleRenderer, SpriteParticleRenderer, BaseParticleEmitter, BoxEmitter, DiscEmitter, LineEmitter, PointEmitter, RectangleEmitter, RingEmitter, SphereSurfaceEmitter, SphereVolumeEmitter, TangentRingEmitter, string, os, DirectNotifyGlobal, sys
...
class Particles(ParticleSystem.ParticleSystem):
    ...
    def printParams(self, file=sys.stdout, targ='self'):
        ...
        if alphaMode == BaseParticleRenderer.BaseParticleRenderer.PRALPHANONE:
            ...
            if alphaMode == BaseParticleRenderer.BaseParticleRenderer.PRALPHAOUT:
                aMode = 'PRALPHAOUT'
            else:
                if alphaMode == BaseParticleRenderer.BaseParticleRenderer.PRALPHAIN:
                    aMode = 'PRALPHAIN'
                else:
                    if alphaMode == BaseParticleRenderer.BaseParticleRenderer.PRALPHAUSER:
                        aMode = 'PRALPHAUSER'
        ...
        if self.rendererType == 'Point':
            ...
            if blendType == PointParticleRenderer.PointParticleRenderer.PPONECOLOR:
                ...
                if blendType == PointParticleRenderer.PointParticleRenderer.PPBLENDLIFE:
                    bType = 'PPBLENDLIFE'
                else:
                    if blendType == PointParticleRenderer.PointParticleRenderer.PPBLENDVEL:
                        bType = 'PPBLENDVEL'
            ...
            if blendMethod == BaseParticleRenderer.BaseParticleRenderer.PPNOBLEND:
                ...
                if blendMethod == BaseParticleRenderer.BaseParticleRenderer.PPBLENDLINEAR:
                    bMethod = 'PPBLENDLINEAR'
                else:
                    if blendMethod == BaseParticleRenderer.BaseParticleRenderer.PPBLENDCUBIC:
        ...
        if emissionType == BaseParticleEmitter.BaseParticleEmitter.ETEXPLICIT:
            ...
            if emissionType == BaseParticleEmitter.BaseParticleEmitter.ETRADIATE:
                eType = 'ETRADIATE'
            else:
                if emissionType == BaseParticleEmitter.BaseParticleEmitter.ETCUSTOM:
                    eType = 'ETCUSTOM'
