...
import FSM, State, ToontownTimer, PatternGameGlobals, ToonHead, AvatarDNA, Char, ArrowKeys, whrandom, ToontownGlobals, string, Localizer
...
class DistributedPatternGame(DistributedMinigame):
    ...
    def load(self):
        ...
        self.roundText = DirectLabel(text='Round 1!', text_fg=self.normalTextColor, frameColor=(1, 1, 1, 0), text_font=ToontownGlobals.getSignFont(), pos=(0.014, 0, -0.84), scale=0.12)
        self.roundText.hide()
    ...
    def enterGetUserInput(self):
        ...
        self.setupTrack = Sequence(Func(self.__setMinnieChat, self.strGo, 0), Func(self.setText, self.roundText, Localizer.PatternGameGo), Func(self.roundText.setScale, 0.3), Func(enableKeys), Func(startTimer), Wait(0.8), Func(self.__clearMinnieChat), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.12), Func(self.setTextFG, self.roundText, self.normalTextColor))
    ...
    def remoteButtonPressed(self, avId, index, wrong):
        if not self.gameFSM.getCurrentState().getName() in ['getUserInput', 'waitForPlayerPatterns']:
            return
