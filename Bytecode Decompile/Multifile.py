...
class Multifile(FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_ls_ptrConstMultifile_ptrOstream(self, out):
        returnValue = libpandaexpress._inPJoxt_Zzn(self.this, out.this)
        return returnValue

    def __overloaded_ls_ptrConstMultifile(self):
    ...
    def ls(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self.__overloaded_ls_ptrConstMultifile()
        else:
            if numArgs == 1:
                import Ostream
                if isinstance(_args[0], Ostream.Ostream):
                    return self.__overloaded_ls_ptrConstMultifile_ptrOstream(_args[0])
                else:
