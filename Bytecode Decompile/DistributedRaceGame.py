...
import FSM, State, Task, ToontownTimer, RaceGameGlobals, ToontownGlobals, Localizer
...
class DistributedRaceGame(DistributedMinigame):
    ...
    def load(self):
        ...
        self.waitingChoicesLabel = DirectLabel(text=Localizer.RaceGameWaitingChoices, text_fg=VBase4(1, 1, 1, 1), relief=None, pos=(-0.6, 0, -0.75), scale=0.075)
        ...
        self.chanceCard.text = OnscreenText('', fg=(1.0, 0, 0, 1), scale=0.14, font=ToontownGlobals.getSignFont(), wordwrap=14, pos=(0.0, 0.2), mayChange=1)
    ...
    def enterMoveAvatars(self, choiceList, positionList, rewardList):
        ...
        for i in range(0, len(choiceList) / self.numPlayers):
            ...
            if longestLerpTime > 0.0:
                ...
                tasks += [Task.pause(0.75), mat, Task.pause(0.75), Task.Task(self.hideNumbers), Task.pause(longestLerpTime - 0.5)]
                tasks += self.showChanceRewards()
            else:
                tasks += [Task.pause(1.0), Task.Task(self.hideNumbers)]
    ...
    def showChanceCard(self, task):
        ...
        task.chanceCard.lerpPosHpr(19.62, 13.41, 13.14, 355.24, 270, 90, 1.0, other=camera, task='cardLerp')
        return Task.done
