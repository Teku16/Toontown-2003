...
def calcMazeTopology(mazeNode):
 ...
 0, 0]):
        ...
        def __crawl(self):
            ...
            while not (leftTerminateCoord and self.curCoord == terminateCoord):
                if not attemptMove():
                    self.direction -= 1
                else:
    ...
    return (
     collisionArray, mazeWidth, mazeHeight, originTX, originTY)
        ...
        while x < mazeWidth - 1:
            if abs(x + 1 - originX) > DEADZONE_X_RADIUS or abs(y + 1 - originY) > DEADZONE_Y_RADIUS:
                if not collisionTable[y][x] and not collisionTable[y + 1][x] and not collisionTable[y][x + 1] and not collisionTable[y + 1][x + 1]:
                    list.append(((x + 1 - originX) * CELL_WIDTH, (y + 1 - originY) * CELL_WIDTH, 0.1))
            x += 1
...
fwritelines(f, [
 '"""' + outputFile + ': GENERATED FILE, DO NOT EDIT"""', '', 'CELL_WIDTH = ' + `CELL_WIDTH`, '', 'mazeNames = ' + `mazeNames`, '', 'mazeData = {}'])
processedMazes = []
