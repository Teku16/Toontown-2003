import types, libpanda, libpandaDowncasts, libpandaexpress, libpandaexpressDowncasts, FFIExternalObject, ReferenceCount, ClearableRegion
...
class DisplayRegion(ReferenceCount.ReferenceCount, ClearableRegion.ClearableRegion, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_computePixels_ptrDisplayRegion(self):
        returnValue = libpanda._inPO9cYZxAV(self.this)
        return returnValue

    def __overloaded_computePixels_ptrDisplayRegion_int_int(self, xSize, ySize):
    ...
    def computePixels(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self.__overloaded_computePixels_ptrDisplayRegion()
        else:
            if numArgs == 2:
                if isinstance(_args[0], types.IntType):
                    if isinstance(_args[1], types.IntType):
                        return self.__overloaded_computePixels_ptrDisplayRegion_int_int(_args[0], _args[1])
                    else:
