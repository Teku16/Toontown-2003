...
import NPCToons, TailorClothesGUI, Localizer, AvatarDNA, ClosetGlobals
...
class DistributedNPCTailor(DistributedNPCToonBase):
    ...
    def popupPurchaseGUI(self, task):
        ...
        if toonbase.localToon.getHeight() > 3.5:
            self.clothesGUI.topLButton.setZ(0.1)
            self.clothesGUI.topRButton.setZ(0.1)
            self.clothesGUI.bottomLButton.setZ(-0.3)
            self.clothesGUI.bottomRButton.setZ(-0.3)
        else:
            self.clothesGUI.topLButton.setZ(0)
            self.clothesGUI.topRButton.setZ(0)
            self.clothesGUI.bottomLButton.setZ(-0.4)
            self.clothesGUI.bottomRButton.setZ(-0.4)
        ...
        camera.setPosHpr(toonbase.localToon, -4.16, 8.25, 2.47, -152.89, 0.0, 0.0)
    ...
    def __enterConfirmLoss(self, finished, which):
        if self.popupInfo == None:
            ...
            self.popupInfo = DirectFrame(parent=hidden, relief=None, state='normal', text=Localizer.STOREOWNER_CONFIRM_LOSS, text_wordwrap=10, textMayChange=0, frameSize=(-1, 1, -1, 1), text_pos=(0, -0.05), geom=getDefaultDialogGeom(), geom_color=ToontownGlobals.GlobalDialogColor, geom_scale=(0.88, 1, 0.55), geom_pos=(0, 0, -0.18), text_scale=0.08)
            DirectButton(self.popupInfo, image=okButtonImage, relief=None, text=Localizer.STOREOWNER_OK, text_scale=0.05, text_pos=(0.0, -0.1), textMayChange=0, pos=(-0.08, 0.0, -0.31), command=self.__handleConfirmLossOK, extraArgs=[finished, which])
            DirectButton(self.popupInfo, image=cancelButtonImage, relief=None, text=Localizer.STOREOWNER_CANCEL, text_scale=0.05, text_pos=(0.0, -0.1), textMayChange=0, pos=(0.08, 0.0, -0.31), command=self.__handleConfirmLossCancel)
            buttons.removeNode()
