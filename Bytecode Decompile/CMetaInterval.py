...
class CMetaInterval(CInterval.CInterval, FFIExternalObject.FFIExternalObject):
    ...
    def __overloaded_popLevel_ptrCMetaInterval_double(self, duration):
        returnValue = libdirect._inPSpsCDQho(self.this, duration)
        return returnValue

    def __overloaded_popLevel_ptrCMetaInterval(self):
        returnValue = libdirect._inPSpsCbB29(self.this)
        return returnValue

    def __overloaded_setIntervalStartTime_ptrCMetaInterval_atomicstring_double___enum__RelativeStart(self, name, relTime, relTo):
        returnValue = libdirect._inPSpsCaNvh(self.this, name, relTime, relTo)
        return returnValue

    def __overloaded_setIntervalStartTime_ptrCMetaInterval_atomicstring_double(self, name, relTime):
        ...
        if numArgs == 2:
            if isinstance(_args[0], types.StringType):
                if isinstance(_args[1], types.FloatType) or isinstance(_args[1], types.IntType):
                    return self.__overloaded_setIntervalStartTime_ptrCMetaInterval_atomicstring_double(_args[0], _args[1])
                else:
                ...
                if isinstance(_args[0], types.StringType):
                    if isinstance(_args[1], types.FloatType) or isinstance(_args[1], types.IntType):
                        if isinstance(_args[2], types.IntType):
                            return self.__overloaded_setIntervalStartTime_ptrCMetaInterval_atomicstring_double___enum__RelativeStart(_args[0], _args[1], _args[2])
                        else:
    ...
    def popLevel(self, *_args):
        numArgs = len(_args)
        if numArgs == 0:
            return self.__overloaded_popLevel_ptrCMetaInterval()
        else:
            if numArgs == 1:
                if isinstance(_args[0], types.FloatType) or isinstance(_args[0], types.IntType):
                    return self.__overloaded_popLevel_ptrCMetaInterval_double(_args[0])
                else:
