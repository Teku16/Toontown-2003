...
import DirectNotifyGlobal, PandaObject, OnscreenText, AvatarDNA, Avatar, ChatManager, StateData, FSM, State, ToontownDialog, re, string, Localizer, NameGenerator, whrandom, PotentialAvatar, NameCheck
MAX_NAME_WIDTH = 7.5
...
class NameShop(PandaObject.PandaObject, StateData.StateData):
    ...
    def enter(self, toon, usedNames, warp):
        ...
        if not self.addedGenderSpecific or self.oldBoy != self.boy:
            ...
            try:
                ...
                self.titleScrollList = self.makeScrollList(nameShopGui, (-0.6, 0, 0.2), (
                 1, 0.8, 0.8, 1), self.allTitles, self.makeLabel, [
                 TextNode.ACenter, 'title'])
                self.firstnameScrollList = self.makeScrollList(nameShopGui, (-0.2, 0, 0.2), (
                 0.8, 1, 0.8, 1), self.allFirsts, self.makeLabel, [
                ...
                self.namePanel = DirectFrame(parent=aspect2d, image=panel, relief='flat', scale=(0.75, 0.7, 0.7), state='disabled', pos=(-0.0163333, 0, 0.103267), frameColor=(1, 1, 1, 0))
                ...
                self.titleCheck = self.makeCheckBox((-0.617, 0, 0.374), Localizer.TitleCheckBox, (
                 0, 0.25, 0.5, 1), self.titleToggle)
                self.firstCheck = self.makeCheckBox((-0.228, 0, 0.374), Localizer.FirstCheckBox, (
                 0, 0.25, 0.5, 1), self.firstToggle)
                self.lastCheck = self.makeCheckBox((0.382, 0, 0.374), Localizer.LastCheckBox, (
        ...
        self.accept('next', self.__createAvatar)
    ...
    def makeScrollList(self, gui, ipos, mcolor, nitems, nitemMakeFunction, nitemMakeExtraArgs):
        it = nitems[:]
        ds = DirectScrolledList(items=it, itemMakeFunction=nitemMakeFunction, itemMakeExtraArgs=nitemMakeExtraArgs, parent=aspect2d, relief=None, command=self.__listsChanged, pos=ipos, scale=0.6, incButton_image=(gui.find('**/triangleButtonUp'), gui.find('**/triangleButtonDwn'), gui.find('**/triangleButtonRllvr'), gui.find('**/triangleButtonUp')), incButton_relief=None, incButton_scale=(1.2, 1.2, -1.2), incButton_pos=(0, 0, -0.545), incButton_image0_color=mcolor, incButton_image1_color=mcolor, incButton_image2_color=mcolor, incButton_image3_color=Vec4(1, 1, 1, 0), decButton_image=(gui.find('**/triangleButtonUp'), gui.find('**/triangleButtonDwn'), gui.find('**/triangleButtonRllvr'), gui.find('**/triangleButtonUp')), decButton_relief=None, decButton_scale=(1.2, 1.2, 1.2), decButton_pos=(0, 0, 0.195), decButton_image0_color=mcolor, decButton_image1_color=mcolor, decButton_image2_color=mcolor, decButton_image3_color=Vec4(1, 1, 1, 0), itemFrame_pos=(-0.2, 0, 0.028), itemFrame_scale=1.0, itemFrame_relief=RAISED, itemFrame_frameSize=(-0.05, 0.48, -0.5, 0.1), itemFrame_frameColor=mcolor, itemFrame_borderWidth=(0.01, 0.01), numItemsVisible=5)
        return ds
        return

    def makeCheckBox(self, npos, ntex, ntexcolor, comm):
        return DirectCheckButton(parent=aspect2d, relief=None, scale=0.1, boxBorder=0.08, boxImage=self.circle, boxImageScale=4, boxImageColor=VBase4(0, 0.25, 0.5, 1), boxRelief=None, pos=npos, text=ntex, text_fg=ntexcolor, text_scale=0.8, text_pos=(0.2, 0), indicator_pos=(-0.566667, 0, -0.045), indicator_image_pos=(-0.26, 0, 0.075), command=comm, text_align=TextNode.ALeft)
        return

    def makeHighlight(self, npos):
        return DirectFrame(parent=aspect2d, relief='flat', scale=(0.552, 0, 0.11), state='disabled', frameSize=(-0.05, 0.48, -0.5, 0.1), borderWidth=(0.01, 0.01), pos=npos, frameColor=(1, 0, 1, 0.4))
    ...
    def firstToggle(self, value):
        ...
        if self.chastise == 2:
            messenger.send('NameShop-mickeyChange', [
             [
              Localizer.ApprovalForName1, Localizer.ApprovalForName2]])
            self.chastise = 0
        if not (self.firstActive or self.lastActive):
            self.firstActive = 1
            messenger.send('NameShop-mickeyChange', [
             [
              Localizer.MustHaveAFirstOrLast1, Localizer.MustHaveAFirstOrLast2]])
            self.chastise = 1
        self.__listsChanged()
        if self.firstActive:
    ...
    def load(self):
        ...
        self.typeNamePanel = DirectFrame(parent=aspect2d, image=typePanel, relief='flat', scale=(0.75, 0.7, 0.7), state='disabled', pos=(-0.0163333, 0, 0.075), frameColor=(1, 1, 1, 0))
        self.typeANameGUIElements.append(self.typeNamePanel)
        self.nameLabel = OnscreenText.OnscreenText(Localizer.PleaseTypeName, parent=aspect2d, style=OnscreenText.ScreenPrompt, pos=(0, 0.53))
        self.typeANameGUIElements.append(self.nameLabel)
        self.typeNotification = OnscreenText.OnscreenText(Localizer.AllNewNames, parent=aspect2d, style=OnscreenText.ScreenPrompt, pos=(0, 0.15))
        self.typeANameGUIElements.append(self.typeNotification)
        self.nameEntry = DirectEntry(parent=aspect2d, relief=None, scale=0.08, entryFont=getToonFont(), width=MAX_NAME_WIDTH, numLines=2, focus=0, cursorKeys=1, pos=(0.0, 0.0, 0.39), text_align=TextNode.ACenter, command=self.__typedAName)
        self.typeANameGUIElements.append(self.nameEntry)
        self.submitButton = DirectButton(parent=aspect2d, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1.2, 0, 1.1), pos=(-0.01, 0, -0.25), text=Localizer.NameShopSubmitButton, text_scale=0.06, text_pos=(0, -0.02), command=self.__typedAName)
        self.typeANameGUIElements.append(self.submitButton)
        self.randomButton = DirectButton(parent=aspect2d, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1.15, 1.1, 1.1), scale=(1.4, 1, 1.4), pos=(0.01, 0, -0.5), text=Localizer.RandomButton, text_scale=0.06, text_pos=(0, -0.02), command=self.__randomName)
        self.pickANameGUIElements.append(self.randomButton)
        self.typeANameButton = DirectButton(parent=aspect2d, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(1, 1.1, 0.9), pos=(0.01, 0, -0.695), scale=(1.6, 1, 1.7), text=Localizer.TypeANameButton, text_scale=0.06, text_pos=(0, -0.02), command=self.__typeAName)
        if toonbase.tcr.productName == 'Terra-DMC':
            self.typeANameButton.hide()
        self.pickANameGUIElements.append(self.typeANameButton)
        self.nameResult = DirectLabel(parent=aspect2d, relief=None, scale=0.12, pos=(0.04, 0, 0.7), text=' \n ', text_align=TextNode.ACenter, text_wordwrap=MAX_NAME_WIDTH)
        ...
        self.titleScrollList = self.makeScrollList(gui, (-0.6, 0, 0.2), (
         1, 0.8, 0.8, 1), self.allTitles, self.makeLabel, [TextNode.ACenter, 'title'])
        self.firstnameScrollList = self.makeScrollList(gui, (-0.2, 0, 0.2), (
         0.8, 1, 0.8, 1), self.allFirsts, self.makeLabel, [TextNode.ACenter, 'first'])
        self.lastprefixScrollList = self.makeScrollList(gui, (0.2, 0, 0.2), (
         0.8, 0.8, 1, 1), self.allPrefixes, self.makeLabel, [TextNode.ARight, 'prefix'])
        self.lastsuffixScrollList = self.makeScrollList(gui, (0.55, 0, 0.2), (
         0.8, 0.8, 1, 1), self.allSuffixes, self.makeLabel, [TextNode.ALeft, 'suffix'])
        gui.removeNode()
        ...
        buttonImage = [
         imageList, imageList]
        buttonText = [Localizer.NameShopPay, Localizer.NameShopPlay]
        self.payDialog = DirectDialog(dialogName='paystate', topPad=0, fadeScreen=0.2, pos=(0, 0.1, 0.1), button_relief=None, text_align=TextNode.ACenter, text=Localizer.NameShopOnlyPaid, buttonTextList=buttonText, buttonImageList=buttonImage, image_color=GlobalDialogColor, buttonValueList=[1, 0], command=self.payAction)
        ...
        self.payDialog.hide()
        buttonText = [
         Localizer.NameShopContinueSubmission, Localizer.NameShopChooseAnother]
        ...
        for x in range(0, 2):
            self.approvalDialog.buttonList[x]['text_pos'] = (
             0, -0.01)
    ...
    def approvalAction(self, value):
        ...
        if value:
            ...
            if self.avExists and not self.newwarp:
                self.avExists = 0
    ...
    def nameShopHandler(self, msgType, di):
        ...
        if msgType == CLIENT_CREATE_AVATAR_RESP:
            ...
            if msgType == CLIENT_SET_NAME_PATTERN_ANSWER:
