...
import PandaObject, Char, Avatar, Toon, LocalToon, AvatarDNA, GenderShop, BodyShop, ColorShop, MakeClothesGUI, NameShop, FSM, State, StateData, ToontownGlobals, whrandom, Task
...
class MakeAToon(PandaObject.PandaObject, StateData.StateData):
    ...
    def load(self):
        ...
        self.guiTopBar = DirectFrame(relief=None, image=gui.find('**/CrtATn_TopBar'), text=Localizer.CreateYourToon, text_font=ToontownGlobals.getSignFont(), text_fg=(0.0, 0.65, 0.35, 1), text_scale=0.18, text_pos=(0, -0.03), pos=(0, 0, 0.86))
        self.guiTopBar.hide()
        self.guiBottomBar = DirectFrame(relief=None, image=gui.find('**/CrtATn_BtmBar'), image_scale=(1.25, 1, 1), pos=(0.01, 0, -0.86))
        self.guiBottomBar.hide()
        self.guiCheckButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(gui.find('**/CrtAtoon_Btn1_UP'), gui.find('**/CrtAtoon_Btn1_DOWN'), gui.find('**/CrtAtoon_Btn1_RLLVR')), pos=(1.165, 0, -0.018), command=self.__handleNext, text=('', Localizer.MakeAToonDone, Localizer.MakeAToonDone), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.08, text_pos=(0, -0.03), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        self.guiCheckButton.hide()
        self.guiCancelButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(gui.find('**/CrtAtoon_Btn2_UP'), gui.find('**/CrtAtoon_Btn2_DOWN'), gui.find('**/CrtAtoon_Btn2_RLLVR')), pos=(-1.179, 0, -0.011), command=self.__handleCancel, text=('', Localizer.MakeAToonCancel, Localizer.MakeAToonCancel), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.08, text_pos=(0, -0.03), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        self.guiCancelButton.hide()
        self.guiNextButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(gui.find('**/CrtAtoon_Btn3_UP'), gui.find('**/CrtAtoon_Btn3_DN'), gui.find('**/CrtAtoon_Btn3_RLVR')), pos=(1.165, 0, -0.018), command=self.__handleNext, text=('', Localizer.MakeAToonNext, Localizer.MakeAToonNext), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.08, text_pos=(0, -0.03), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        self.guiNextButton.hide()
        self.guiLastButton = DirectButton(parent=self.guiBottomBar, relief=None, image=(gui.find('**/CrtAtoon_Btn3_UP'), gui.find('**/CrtAtoon_Btn3_DN'), gui.find('**/CrtAtoon_Btn3_RLVR')), image_scale=(-1.0, 1, 1), pos=(0.825, 0, -0.018), command=self.__handleLast, text=('', Localizer.MakeAToonLast, Localizer.MakeAToonLast), text_font=ToontownGlobals.getInterfaceFont(), text_scale=0.08, text_pos=(0, -0.03), text_fg=(1, 1, 1, 1), text_shadow=(0, 0, 0, 1))
        self.guiLastButton.hide()
        gui.removeNode()
    ...
    def charRunFromLeft(self, char, newpos, newhpr, timelimit=1.6, comm=None, eA=[]):
        ...
        myInterval.append(FunctionInterval(char.setPosHpr, extraArgs=[-12 + newpos[0], -4.5, 0, 270, 0, 0]))
        if not self.slide:
            myInterval.append(FunctionInterval(char.loop, extraArgs=['run']))
        ...
        myInterval.append(FunctionInterval(char.loop, extraArgs=['neutral']))
        if comm:
    ...
    def charRunFromRight(self, char, newpos, newhpr, timelimit=1, comm=None, eA=[]):
        myInterval = []
        myInterval.append(FunctionInterval(self.host.clearChat))
        myInterval.append(FunctionInterval(char.setPosHpr, extraArgs=[9.5 + newpos[0], -4.5, 0, 80, 0, 0]))
        if not self.slide:
            myInterval.append(FunctionInterval(char.loop, extraArgs=['run']))
        myInterval.append(LerpPosInterval(char, duration=timelimit, pos=newpos))
        myInterval.append(LerpHprInterval(char, duration=0.3, hpr=newhpr))
    ...
    def genderShopOpening(self, task):
        self.host = self.minnie
        self.hostSez([Localizer.GenderShopQuestionMinnie])
        self.host = self.mickey
        self.hostSez([Localizer.GenderShopQuestionMickey])
        return Task.done
    ...
    def __handleGenderShopDone(self):
        ...
        if self.toon.style.gender == 'm':
            ...
            gendershopInterval.append(FunctionInterval(self.charSez, extraArgs=[self.minnie, Localizer.GenderShopSeeYou]))
            self.host = self.mickey
            self.hostSez([Localizer.GenderShopFollow])
        else:
            self.timeToGetOff = self.leftTime
            self.mickey.clearChat()
            gendershopInterval.append(FunctionInterval(self.charSez, extraArgs=[self.mickey, Localizer.GenderShopSeeYou]))
            self.host = self.minnie
            self.hostSez([Localizer.GenderShopFollow])
    ...
    def bodyShopOpening(self):
        self.hostSez([Localizer.CreateYourToon])
        self.bs.showButtons()
    ...
    def bodyShopDialogTask(self, task):
        self.hostSez([Localizer.MakeAToonClickForNextScreen])
        return Task.done
    ...
    def colorShopOpening(self):
        self.hostSez([Localizer.PaintYourToon])
    ...
    def colorShopDialogTask(self, task):
        self.hostSez([Localizer.MakeAToonYouCanGoBack])
        return Task.done
    ...
    def clothesShopOpening(self):
        self.hostSez([Localizer.PickClothes])
        self.guiNextButton.show()
        self.guiLastButton.show()
    ...
    def nameShopOpening(self, task):
        ...
        if NAMESHOP not in self.shopsVisited:
            ...
            self.hostSez([Localizer.MakeAFunnyName])
        else:
            self.hostSez([Localizer.MakeAToonLastStep, Localizer.PickANameYouLike])
        return Task.done
    ...
    def enterNameShop(self):
        ...
        if self.progressing:
            self.hostTrack = Parallel(self.charRunFromLeft(self.host, newpos=Point3(4.9, -1.9, 0), newhpr=self.hostHpr, comm=self.ns.enter, eA=[self.toon, self.nameList, self.warp]), self.charRunFromLeft(self.toon, newpos=Point3(-4.1, -2.0, 0), newhpr=self.toonHpr))
            self.hostTrack.play()
