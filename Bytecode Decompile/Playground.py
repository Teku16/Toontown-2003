...
import DirectNotifyGlobal, Place, PandaObject, StateData, FSM, State, Task, DeathForceAcknowledge, HealthForceAcknowledge, TutorialForceAcknowledge, NPCForceAcknowledge, Trolley, ToontownDialog, ToontownGlobals, Localizer
    ...
    def enterTeleportIn(self, requestStatus):
        if len(toonbase.localToon.quests) == 1 and toonbase.localToon.quests[0][0] == NPCForceAcknowledge.TROLLEY_QUEST:
            ...
            if toonbase.localToon.quests[0][4] < 1:
                ...
                heading = 45.0
                scale = 0.007
            self.dialog = ToontownDialog.ToontownDialog(text=msg, command=self.__cleanupDialog, style=ToontownDialog.Acknowledge)
            if not icon.isEmpty():
