...
import FSM, State, ToontownTimer, ToonHead, AvatarDNA, Suit, Char, ArrowKeys, whrandom, ToontownGlobals, string, Localizer, TugOfWarGameGlobals, Rope, Splash, Ripples, Localizer, MinigamePowerMeter
...
class DistributedTugOfWarGame(DistributedMinigame):
    ...
    def initCamera(self):
        birdseyePosHpr = [
         1.95461, 18.4891, 38.4646, 1.18185, -87.5308, -2.13716]
        introPosHpr = [
         None] * 2
        introPosHpr[0] = [VBase3(-11.4427, 9.03559, 2.80094), VBase3(-49.104, -0.732374, -0.2046)]
        introPosHpr[1] = [VBase3(16.9291, 13.9302, 2.64282), VBase3(66.9685, -6.195, 2.04122)]
        gameCamHpr = VBase3(-1.13, 1.042, -0.061)
    ...
    def sendGoSignal(self, index):
        ...
        self.setupTrack = Sequence(Func(self.setText, self.roundText, Localizer.TugOfWarGameReady), Wait(1.5), Func(base.playSfx, self.whistleSound), Func(self.setText, self.roundText, Localizer.TugOfWarGameGo), Func(self.roundText.setScale, 0.3), Wait(1.5), Func(startTimer), Func(enableKeys), Func(self.gameFSM.request, 'tug'), Func(self.setText, self.roundText, ' '), Func(self.roundText.setScale, 0.2))
    ...
    def sendStopSignal(self, winners, losers, tieers):
        ...
        if self.suit:
            if self.suitId in winners:
                ...
                if randInt < 3:
                    ...
                    reactIntervals.append(Sequence(Func(self.suit.loop, 'victory'), Wait(2.6), LerpPosInterval(self.suit, duration=2, pos=newPos), oopsTrack, Func(self.suit.loop, 'neutral')))
        for avId in self.avIdList:
            toon = self.getAvatar(avId)
            toon.loop('neutral')
            if avId in winners:
        ...
        if self.localAvId in winners:
            ...
            if self.localAvId in losers:
                exitIntervals.append(Func(self.setText, self.roundText, Localizer.TugOfWarGameEnd))
                exitIntervals.append(Wait(4.8 + suitSlipTime))
            else:
