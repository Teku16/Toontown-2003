...
import PandaObject, Task, string
_OPT_DEFAULT = 0
_OPT_VALUE = 1
_OPT_FUNCTION = 2
        ...
        for name, default, function in optionDefs:
            if '_' not in name:
                if not optionInfo_has_key(name):
                    ...
                    if optionInfo[name][FUNCTION] is None:
        ...
        if len(kw) == 0:
            if option is None:
                ...
                for option, config in self._optionInfo.items():
                    rtn[option] = (
                     option, config[_OPT_DEFAULT], config[_OPT_VALUE])
...
class DirectGuiWidget(DirectGuiBase, NodePath):
    ...
    def __init__(self, parent=aspect2d, **kw):
        optiondefs = (
         (
          'pgFunc', PGItem, None), ('numStates', 1, None), ('invertedFrames', (), None), ('sortOrder', 0, None), ('state', NORMAL, self.setState), ('relief', FLAT, self.setRelief), ('borderWidth', (0.1, 0.1), self.setBorderWidth), ('frameSize', None, self.setFrameSize), ('frameColor', (0.8, 0.8, 0.8, 1), self.setFrameColor), ('pad', (0, 0), self.resetFrameSize), ('guiId', None, INITOPT), ('pos', None, INITOPT), ('scale', None, INITOPT), ('color', None, INITOPT), ('suppressMouse', 1, INITOPT), ('suppressKeys', 0, INITOPT), ('enableEdit', 1, INITOPT))
        ...
        if self['scale']:
            scale = self['scale']
            if isinstance(scale, Vec3) or type(scale) == types.IntType or type(scale) == types.FloatType:
                self.setScale(scale)
            else:
    ...
    def setState(self):
        if type(self['state']) == type(0):
            ...
            if self['state'] == NORMAL or self['state'] == 'normal':
    ...
    def setRelief(self, fSetStyle=1):
        ...
        if relief == None:
            ...
            if type(relief) == types.StringType:
