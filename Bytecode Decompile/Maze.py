...
class Maze:
    ...
    def isWalkable(self, tX, tY, rejectList=()):
        if tX <= 0 or tY <= 0 or tX >= self.width or tY >= self.height:
            return 0
        return not self.collisionTable[tY][tX] and not self.collisionTable[tY - 1][tX] and not self.collisionTable[tY][tX - 1] and not self.collisionTable[tY - 1][tX - 1] and not (tX, tY) in rejectList

    def tile2world(self, TX, TY):
        return [
         (TX - self.originTX) * MazeData.CELL_WIDTH, (TY - self.originTY) * MazeData.CELL_WIDTH]

    def world2tile(self, x, y):
        return [
         int(x / MazeData.CELL_WIDTH + self.originTX), int(y / MazeData.CELL_WIDTH + self.originTY)]