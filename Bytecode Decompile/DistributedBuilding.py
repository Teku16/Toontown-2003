...
import ToontownGlobals, DirectNotifyGlobal, FSM, DistributedObject, whrandom, AvatarDNA, Localizer, DelayDelete, Emote
...
class DistributedBuilding(DistributedObject.DistributedObject):
    ...
    def __init__(self, cr):
        ...
        self.fsm = FSM.FSM('DistributedBuilding', [
         ...
         State.State('suit', self.enterSuit, self.exitSuit, [
          'waitForVictors', 'becomingToon'])], 'off', 'off')
    ...
    def enterWaitForVictors(self, ts):
        ...
        if victorCount == 1:
            ...
            if anyOthers:
                self.waitingMessage = DirectLabel(text=Localizer.BuildingWaitingForVictors, text_fg=VBase4(1, 1, 1, 1), text_align=TextNode.ACenter, relief=None, pos=(0, 0, 0.35), scale=0.1)
        else:
            if victorCount == 0:
                pass
            else:
    ...
    def animToSuit(self, timeStamp):
        ...
        for i in sideBldgNodes:
            ...
            if name[0] == 's':
                ...
                showIvals.extend(self.createBounceIvals(i, 2, 0.65, TO_SUIT_BLDG_TIME - timeForDrop, slowInitBounce=1.0))
                if not soundPlayed:
                ...
                if name[0] == 't':
                    ...
                    hideIvals.append(LerpFunctionInterval(self.adjustColorScale, fromData=1, toData=0.25, duration=timeTillSquish, extraArgs=[i]))
    ...
    def animToToon(self, timeStamp):
        ...
        for i in bldgNodes:
            ...
            if name[0] == 's':
                ...
                hideIvals.extend(self.createBounceIvals(i, 3, 1.2, TO_TOON_BLDG_TIME * 0.05, slowInitBounce=0.0))
                hideIvals.extend(self.createBounceIvals(i, 5, 0.8, TO_TOON_BLDG_TIME * 0.1, slowInitBounce=0.0))
                hideIvals.extend(self.createBounceIvals(i, 7, 1.2, TO_TOON_BLDG_TIME * 0.17, slowInitBounce=0.0))
                hideIvals.extend(self.createBounceIvals(i, 9, 1.2, TO_TOON_BLDG_TIME * 0.18, slowInitBounce=0.0))
                realScale = i.getScale()
                hideIvals.append(LerpScaleInterval(i, TO_TOON_BLDG_TIME * 0.1, Vec3(realScale[0], realScale[1], 0.01)))
                if landmark:
