...
import ToontownTimer, ToontownGlobals, ToontownBattleGlobals, Localizer
...
class TrackPoster(DirectFrame):
    ...
    def __init__(self, trackId, callback):
        ...
        self.chooseButton = DirectButton(parent=self.poster, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text=Localizer.TrackChoiceGuiChoose, text_scale=0.06, text_pos=(0, -0.02), command=callback, extraArgs=[trackId], pos=(0, 0, -0.16), scale=0.8)
...
class TrackChoiceGui(DirectFrame):
    ...
    def __init__(self, tracks, timeout):
        DirectFrame.__init__(self, relief=None, geom=getDefaultDialogGeom(), geom_color=Vec4(0.8, 0.6, 0.4, 1), geom_scale=(1.5, 1, 0.9), geom_hpr=(0, 0, 90), pos=(-0.85, 0, 0))
        self.initialiseoptions(TrackChoiceGui)
        guiButton = loader.loadModelOnce('phase_3/models/gui/quit_button')
        self.cancelButton = DirectButton(parent=self, relief=None, image=(guiButton.find('**/QuitBtn_UP'), guiButton.find('**/QuitBtn_DN'), guiButton.find('**/QuitBtn_RLVR')), image_scale=(0.7, 1, 1), text=Localizer.TrackChoiceGuiCancel, pos=(0.15, 0, -0.625), text_scale=0.06, text_pos=(0, -0.02), command=self.chooseTrack, extraArgs=[-1])
        guiButton.removeNode()
