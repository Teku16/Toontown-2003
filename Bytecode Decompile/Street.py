...
import DirectNotifyGlobal, Place, PandaObject, StateData, FSM, State, Task, Toon, BattleParticles, Elevator, QuietZoneState, ZoneUtil, ToontownGlobals, HouseGlobals, Localizer
...
class Street(Place.Place):
    ...
    def __init__(self, hood, parentFSM, doneEvent):
        ...
        self.fsm = FSM.FSM('Street', [
         ...
         State.State('walk', self.enterWalk, self.exitWalk, [
          'stickerBook', 'WaitForBattle', 'battle', 'DFA', 'doorOut', 'elevator', 'tunnelIn', 'tunnelOut', 'teleportOut', 'quest']),
         State.State('stickerBook', self.enterStickerBook, self.exitStickerBook, [
          'walk', 'battle', 'DFA', 'WaitForBattle']),
    ...
    def __teleportOutDone(self, requestStatus):
        ...
        if hoodId == self.loader.hood.id and shardId == None:
            if zoneId == self.__zoneId:
                self.fsm.request('teleportIn', [requestStatus])
            else:
                if requestStatus['where'] == 'street' and ZoneUtil.getBranchZone(zoneId) == self.loader.branchZone:
    ...
    def handleQuietZoneDone(self):
        ...
        self.fsm.request(how, [toonbase.tcr.handlerArgs])
    ...
    def enterZone(self, newZone):
        ...
        if self.__zoneId != None:
            for i in self.loader.nodeDict[self.__zoneId]:
                if newZoneId:
        ...
        if newZoneId != None:
            for i in self.loader.nodeDict[newZoneId]:
                if self.__zoneId:
                    if i not in self.loader.nodeDict[self.__zoneId]:
        ...
        if newZoneId != self.__zoneId:
            if visualizeZones:
                if self.__zoneId != None:
                    self.loader.zoneDict[self.__zoneId].clearColor()
                if newZoneId != None:
